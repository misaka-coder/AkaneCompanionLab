...
class _AnthropicStream:
    ...
    def __iter__(self):
        ...
        try:
            for raw_line in self._response.iter_lines(decode_unicode=False):
                ...
                if not line:
                    self._capture_content(event_name, data_lines)
    ...
    def _capture_content(self, event_name, lines):
        if not lines:
            return
        payload = json.loads("\n".join(lines))
        index = payload.get("index", 0)
        if event_name == "content_block_start":
            self._blocks[index] = deepcopy(payload.get("content_block") or {})
        elif event_name == "content_block_delta" and index in self._blocks:
            delta = payload.get("delta") or {}
            kind = delta.get("type")
            field = {"text_delta": "text", "thinking_delta": "thinking", "signature_delta": "signature"}.get(kind)
            if field:
                self._blocks[index][field] = self._blocks[index].get(field, "") + delta.get(field, "")
            elif kind == "input_json_delta":
                self._json_parts[index] = self._json_parts.get(index, "") + delta.get("partial_json", "")
        elif event_name == "content_block_stop" and index in self._json_parts:
            self._blocks[index]["input"] = json.loads(self._json_parts[index])
        self.provider_content = [deepcopy(self._blocks[k]) for k in sorted(self._blocks)]
...
def _convert_openai_tools_to_anthropic_tools(value) -> list[dict]:
    ...
    for raw in value:
        ...
        tool = {
            "name": name,
            "description": str(function.get("description") or "").strip() or f"Call Akane tool {name}.",
...
def _convert_messages(messages: list) -> tuple[str, list]:
    ...
    for item in messages:
        ...
        converted.append(
            {
                "role": role,
                "content": wire_data(item, "anthropic") or _convert_content_blocks(content),
            }
        )
...
def _build_openai_style_response(payload: dict, model: str):
    ...
    return SimpleNamespace(
        ...
        provider_content=deepcopy(raw_content),
        choices=[
            SimpleNamespace(
                index=0,
