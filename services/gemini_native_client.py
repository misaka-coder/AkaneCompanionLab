...
import requests
from .provider_continuation import wire_data
            ...
            for payload in _iter_sse_payloads(self._response):
                ...
                for part in parts if isinstance(parts, list) else []:
                    ...
                    if not isinstance(function_call, dict):
                        self.provider_content.append(deepcopy(part))
                        continue
                    ...
                    if fingerprint in emitted_tool_call_fingerprints:
                        previous = self.provider_content[part_indexes[fingerprint]]
                        previous.update({k: deepcopy(v) for k, v in part.items() if k != "functionCall"})
                        continue
                    emitted_tool_call_fingerprints.add(fingerprint)
                    if not call_id:
                        call_id = f"call_gemini_{uuid.uuid4().hex[:16]}"
                    saved = deepcopy(part)
                    saved["functionCall"]["id"] = call_id
                    part_indexes[fingerprint] = len(self.provider_content)
                    self.provider_content.append(saved)
                    tool_calls.append(
                        {
...
def gemini_thinking_config(model: str, effort: str) -> dict[str, Any]:
    """Map the host's optional effort setting; an unset setting keeps defaults.

    2.5 has a token budget rather than levels. The budgets below are host
    presets, not claimed to be Google's definition of equivalent effort.
    The older DeepSeek-only enabled/disabled switch does not control Gemini.
    """
    if not effort:
        return {}
    name = str(model).lower()
    if "gemini-2.5" in name:
        if effort == "none" and "pro" in name:
            raise ValueError("gemini_pro_thinking_cannot_be_disabled")
        budgets = {"none": 0, "minimal": 128, "low": 1024, "medium": 4096,
                   "high": 8192, "xhigh": 16384, "max": 24576}
        if effort in budgets:
            return {"thinkingBudget": budgets[effort]}
    elif "gemini-3" in name and effort in {"minimal", "low", "medium", "high"}:
        # Model-specific restrictions remain explicit provider errors.
        return {"thinkingLevel": effort}
    raise ValueError("gemini_reasoning_effort_unsupported_for_model")
...
def _build_gemini_payload(kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    thinking = kwargs.get("thinking_config")
    if isinstance(thinking, dict):
        payload.setdefault("generationConfig", {})["thinkingConfig"] = deepcopy(thinking)
...
def _convert_openai_messages(value: Any) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for raw in messages:
        ...
        saved_parts = wire_data(raw, "gemini") if role == "assistant" else None
        if saved_parts:
            for saved_part in saved_parts:
                call = saved_part.get("functionCall", {})
                if call.get("id"):
                    tool_names_by_id[call["id"]] = call.get("name", "")
        ...
        declarations.append(
            {
                ...
                "parametersJsonSchema": _json_schema_without_host_extensions(parameters),
            }
        )
        seen.add(name)
    return declarations
...
def _json_schema_without_host_extensions(schema: Any) -> Any:
    """Remove host annotations, preserving JSON Schema constraints and literals.

    Native FunctionDeclaration has a JSON Schema field. Do not downgrade it
    to the narrower OpenAPI Schema message or rewrite oneOf into anyOf.
    """
    if not isinstance(schema, dict):
        return deepcopy(schema)
    maps = {"properties", "patternProperties", "$defs", "definitions", "dependentSchemas"}
    single = {"items", "contains", "additionalProperties", "unevaluatedProperties", "unevaluatedItems",
              "propertyNames", "not", "if", "then", "else", "additionalItems"}
    lists = {"allOf", "anyOf", "oneOf", "prefixItems"}
    result = {}
    for key, value in schema.items():
        if key.startswith("x-capcore-"):
            continue
        if key in maps and isinstance(value, dict):
            result[key] = {name: _json_schema_without_host_extensions(child) for name, child in value.items()}
        elif key in single or key in lists:
            result[key] = ([_json_schema_without_host_extensions(child) for child in value]
                           if isinstance(value, list) else _json_schema_without_host_extensions(value))
        else:
            result[key] = deepcopy(value)
    return result


def sanitize_openai_tools_for_gemini(value: Any) -> list[dict[str, Any]]:
    """Keep OpenAI tool envelopes while restricting schemas to Gemini fields."""
    if not isinstance(value, list):
        return []
...
def _normalize_gemini_schema(value: Any) -> Any:
    """Legacy schema subset for Gemini's OpenAI-compatible route only."""
    ...
    for key, item in value.items():
        ...
        if clean_key == "oneOf":
            clean_key = "anyOf"
        if clean_key == "const":
            normalized["enum"] = [deepcopy(item)]
            continue
        if clean_key not in _GEMINI_SCHEMA_KEYS:
            continue
        if clean_key == "properties":
            if isinstance(item, dict):
                normalized["properties"] = {
                    str(property_name): _normalize_gemini_schema(property_schema)
        ...
        if clean_key == "type":
            if isinstance(item, list):
                ...
                if len(non_null) == 1:
                    ...
                    normalized["anyOf"] = [{"type": raw_type} for raw_type in non_null]
                continue
            raw_type = str(item or "").strip().lower()
            normalized["type"] = raw_type
            continue
        if clean_key in {"anyOf", "items"}:
            normalized[clean_key] = _normalize_gemini_schema(item)
        else:
            # Schema metadata and literal values (for example ``default``)
            # are data, not nested schemas. Preserve their shape verbatim.
            normalized[clean_key] = deepcopy(item)
    if nullable:
