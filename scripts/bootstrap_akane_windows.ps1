...
function Get-AkaneDefaultDataRoot {
...
function Read-AkaneActiveProfile {
    param([string]$DefaultDataRoot)

    $selectorPath = ([string]$env:AKANE_ACTIVE_PROFILE_FILE).Trim()
    if (-not $selectorPath) {
        $selectorPath = Join-Path $DefaultDataRoot "config\active-profile.json"
    } elseif (-not [System.IO.Path]::IsPathRooted($selectorPath)) {
        $selectorPath = [System.IO.Path]::GetFullPath((Join-Path $DefaultDataRoot $selectorPath))
    } else {
        $selectorPath = [System.IO.Path]::GetFullPath($selectorPath)
    }
    ...
    try {
        $profile = Get-Content -LiteralPath $selectorPath -Raw -Encoding UTF8 | ConvertFrom-Json
    } catch {
        throw "active_profile_invalid_json"
    }
    if ($null -eq $profile -or $profile.schema_version -ne 1) {
        throw "active_profile_schema_unsupported"
    }
    $instanceId = ([string]$profile.instance_id).Trim()
    if (-not (Test-AkaneSafeInstanceId -InstanceId $instanceId)) {
        throw "active_profile_instance_invalid"
    }
    $dataRoot = ([string]$profile.data_root).Trim()
    if (-not $dataRoot -or -not [System.IO.Path]::IsPathRooted($dataRoot)) {
        throw "active_profile_data_root_required"
    }
    $dataRoot = [System.IO.Path]::GetFullPath($dataRoot)
    $backendPort = 0
    if (-not [int]::TryParse(([string]$profile.backend_port).Trim(), [ref]$backendPort) -or $backendPort -lt 1 -or $backendPort -gt 65535) {
        throw "active_profile_backend_port_invalid"
    }
    $environmentFiles = @()
    foreach ($rawPath in @($profile.environment_files)) {
        $path = ([string]$rawPath).Trim()
        if (-not $path) { continue }
        if (-not [System.IO.Path]::IsPathRooted($path)) {
            $path = [System.IO.Path]::GetFullPath((Join-Path $DefaultDataRoot $path))
        } else {
            $path = [System.IO.Path]::GetFullPath($path)
        }
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "active_profile_environment_file_missing"
        }
...
function Ensure-PythonEnvironment {
    ...
    # A normal desktop launch must stay a start operation.  A requirements
    # hash change is maintenance work and may require a network/index or a
    # complete offline wheelhouse, so only an incomplete environment or the
    # explicit -ForcePythonInstall flag may trigger installation.
    $requiresInstall = $ForcePythonInstall -or -not $importsReady
    ...
    if ($ReadOnly) {
        ...
        if ($installedHash -ne $requirementsHash) {
            Write-AkaneStep "WARN" "requirements.txt changed after the last bootstrap; run with -ForcePythonInstall to update dependencies."
        }
        return $venvPython
    }
    ...
    } elseif ($installedHash -ne $requirementsHash) {
        Write-AkaneStep "WARN" "Python imports are usable, but requirements changed; run with -ForcePythonInstall to update dependencies."
    } else {
...
function Resolve-LaunchMode {
    ...
    # Auto is the ordinary desktop contract. A missing toolchain is a clear
    # preparation failure, not permission to silently switch the user's client
    # and its visible state to Web. Web remains an explicit -Mode Web choice.
    Write-AkaneStep "WARN" ("Desktop build tools are incomplete ({0}); desktop launch will report the missing prerequisites." -f ($toolchain.Missing -join ", "))
    return "Desktop"
}
...
function Start-WebMode {
    ...
    if ($RestartBackend) { $arguments.RestartBackend = $true }
    & (Join-Path $Root "start_akane_next.ps1") @arguments
...
function Start-DesktopMode {
    ...
    if ($RestartBackend) { $launchParameters.RestartBackend = $true }
    if ($CloudSatellite) {
...
$defaultDataRoot = Get-AkaneDefaultDataRoot
$activeProfile = $null
$profileEligible = (
    -not $instanceIdWasBound -and
    -not $dataRootWasBound -and
    -not $envFileWasBound
)
if ($profileEligible) {
    $activeProfile = Read-AkaneActiveProfile -DefaultDataRoot $defaultDataRoot
    if ($null -ne $activeProfile) {
        foreach ($environmentFile in @($activeProfile.EnvironmentFiles)) {
            $null = Import-AkaneEnvFile -Path $environmentFile
        }
        Write-AkaneStep "INFO" ("Using persisted active profile: {0}." -f $activeProfile.InstanceId)
    }
}
$resolvedEnvFile = ""
...
if ($null -ne $activeProfile -and -not $backendPortWasBound) {
    $BackendPort = $activeProfile.BackendPort
} elseif (-not $backendPortWasBound -and -not [string]::IsNullOrWhiteSpace([string]$env:COMPANION_PORT)) {
    $configuredPort = 0
...
try {
    $dataStatus = Initialize-AkaneDataRoot `
        ...
        -ReadOnly:($CheckOnly -or $Action -in @("Status", "Stop", "Doctor"))
    ...
    if ($Action -ne "Start") {
        if ($CloudSatellite) { throw "lifecycle_action_requires_local_backend" }
        $launcherParameters = @{
            Action = $Action
            InstanceId = $resolvedInstanceId
        ...
        if ($resolvedEnvFile) { $launcherParameters.EnvFile = $resolvedEnvFile }
        & (Join-Path $projectRoot "start_akane_next.ps1") @launcherParameters
        $launcherExitVariable = Get-Variable -Name LASTEXITCODE -ErrorAction SilentlyContinue
        $launcherExitCode = if ($null -ne $launcherExitVariable) { [int]$launcherExitVariable.Value } else { 0 }
    ...
    if ($CheckOnly) {
        ...
        Start-DesktopMode -Root $projectRoot -Port $BackendPort -InstanceId $resolvedInstanceId -DataRoot $dataStatus.Root -EnvFile $resolvedEnvFile -CloudSatellite:$CloudSatellite -BackendUrl $BackendUrl -OpenModelSettings:(-not $envStatus.LlmConfigured) -RestartBackend:$RestartBackend
    } else {
        Start-WebMode -Root $projectRoot -Port $BackendPort -InstanceId $resolvedInstanceId -DataRoot $dataStatus.Root -EnvFile $resolvedEnvFile -OpenModelSettings:(-not $envStatus.LlmConfigured) -RestartBackend:$RestartBackend
    }
