...
$botProfiles = @{
    personal = [pscustomobject]@{
        ...
        WebUiConfig = "/var/lib/akane/personal/napcat/config/webui.json"
    }
    finance = [pscustomobject]@{
        ...
        WebUiConfig = "/var/lib/akane/finance/napcat/config/webui.json"
    }
}
...
if ! sudo -n test -r "$webui_config"; then
    echo AKANE_RESULT=PREPARE_FAILED
    echo AKANE_REASON=webui_config_missing
    exit 31
fi
...
while [ "$(date +%s)" -lt "$deadline" ]; do
    ...
    # Newer NapCat versions expose the login QR as a short-lived WebUI URL
    # instead of writing /app/napcat/cache/qrcode.png. Render that URL locally
    # on the VPS so the login URL is not sent to a third-party QR service.
    webui_port=$(sudo -n docker port "$container" 6099/tcp 2>/dev/null | sed -n 's/.*:\([0-9][0-9]*\)$/\1/p' | head -n 1)
    webui_token=$(sudo -n jq -r '.token // empty' "$webui_config" 2>/dev/null || true)
    if [ -n "$webui_port" ] && [ -n "$webui_token" ]; then
        webui_hash=$(printf '%s.napcat' "$webui_token" | sha256sum | awk '{print $1}')
        webui_credential=$(curl -sS --max-time 5 -X POST \
            -H 'Content-Type: application/json' \
            --data-binary "{\"hash\":\"$webui_hash\"}" \
            "http://127.0.0.1:$webui_port/api/auth/login" 2>/dev/null \
            | jq -r '.data.Credential // empty' 2>/dev/null || true)
        if [ -n "$webui_credential" ]; then
            qr_url=$(curl -sS --max-time 5 -X POST \
                -H "Authorization: Bearer $webui_credential" \
                "http://127.0.0.1:$webui_port/api/QQLogin/CheckLoginStatus" 2>/dev/null \
                | jq -r '.data.qrcodeurl // empty' 2>/dev/null || true)
            if [ -z "$qr_url" ] && [ "$webui_restart_requested" -eq 0 ]; then
                curl -sS --max-time 5 -X POST \
                    -H "Authorization: Bearer $webui_credential" \
                    "http://127.0.0.1:$webui_port/api/QQLogin/RestartNapCat" >/dev/null 2>&1 || true
                webui_restart_requested=1
                sleep 2
                continue
            fi
            if [ -n "$qr_url" ]; then
                if ! sudo -n test -s "$remote_qr"; then
                    if ! sudo -n test -d /opt/akane/tools/qrgen; then
                        sudo -n mkdir -p /opt/akane/tools/qrgen
                        sudo -n python3 -m pip install --quiet --disable-pip-version-check --target /opt/akane/tools/qrgen 'qrcode[pil]' >/dev/null 2>&1 || true
                    fi
                    sudo -n env QR_URL="$qr_url" QR_OUT="$remote_qr" \
                        PYTHONPATH=/opt/akane/tools/qrgen python3 -c \
                        'import os, qrcode; qrcode.make(os.environ["QR_URL"]).save(os.environ["QR_OUT"])' \
                        >/dev/null 2>&1 || true
                    sudo -n chmod 0644 "$remote_qr" >/dev/null 2>&1 || true
                fi
                if sudo -n test -s "$remote_qr"; then
                    qr_size=$(sudo -n stat -c %s "$remote_qr" 2>/dev/null || printf 0)
                    if [ "$qr_size" -ge 100 ]; then
