...
function Get-AkaneBackendPortDecision {
    ...
    # A normal start is an activation request. Reusing a verified, managed
    # backend keeps the user's session, memory writer and channel leases alive.
    # Restart is an explicit maintenance action, never an implicit consequence
    # of double-clicking the launcher. Keep ReuseBackend for compatibility with
    # older callers, but do not require it for the safe default.
    if ($ManagedProcess -and ($ReuseBackend -or -not $RestartBackend)) {
        return "reuse"
    }
    if ($ManagedProcess -and $RestartBackend) {
        return "stop"
    }
    return "reject"
}
