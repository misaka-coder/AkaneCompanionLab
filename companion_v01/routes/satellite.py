...
def build_satellite_router(
    *,
    ...
    @router.websocket("/capabilities/satellite/presentation/ws")
    async def satellite_presentation_websocket(websocket: WebSocket) -> None:
        await satellite_service.handle_websocket(websocket, presentation_only=True)
