    ...
    async def _try_synthesize_petdesk_tts(
        *,
        ...
        if failure:
            ...
            _log("petdesk_tts_skipped", reason=failure)
            return None
        ...
        try:
            synthesized = await synthesize_tts_resolution(
                ...
                text=text,
                payload=payload,
                default_media_type="audio/wav" if resolution["activeProviderId"] == GPT_SOVITS_PROVIDER_ID else "audio/mpeg",
            )
            ...
            _log(
                "petdesk_tts_failed",
                reason=_safe_tts_reason(getattr(exc, "reason", "tts_synthesis_failed")),
