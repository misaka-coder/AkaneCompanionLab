...
def build_control_center_snapshot_runtime_providers(
    *,
    ...
    return {
        ...
        "executionPolicy": lambda _context: _build_snapshot_execution_policy(engine=engine),
        "resourcePolicy": lambda _context: _build_snapshot_resource_policy(engine=engine),
...
def _build_snapshot_execution_policy(*, engine: Any) -> dict[str, Any]:
    broker = getattr(engine, "executor_broker", None)
    pipeline = getattr(broker, "execution_policies", None)
    snapshot = getattr(pipeline, "snapshot", None)
    if not callable(snapshot):
        return {"ok": False, "status": "unavailable", "reason": "execution_policy_unavailable"}
    try:
        value = snapshot()
    except Exception:
        return {"ok": False, "status": "unavailable", "reason": "execution_policy_snapshot_failed"}
    if not isinstance(value, dict):
        return {"ok": False, "status": "unavailable", "reason": "execution_policy_snapshot_invalid"}
    return {"ok": True, "status": "available", "policy": value}


def _build_snapshot_resource_policy(*, engine: Any) -> dict[str, Any]:
    broker = getattr(engine, "executor_broker", None)
    policy = getattr(broker, "resource_policy", None)
    snapshot = getattr(policy, "as_dict", None)
    if not callable(snapshot):
        return {"ok": False, "status": "unavailable", "reason": "resource_policy_unavailable"}
    try:
        value = snapshot()
    except Exception:
        return {"ok": False, "status": "unavailable", "reason": "resource_policy_snapshot_failed"}
    if not isinstance(value, dict):
        return {"ok": False, "status": "unavailable", "reason": "resource_policy_snapshot_invalid"}
    return {"ok": True, "status": "available", "policy": value}
