...
EXEC_RUN_TOOL_SPEC = CapabilityToolSpec(
    ...
    description=(
        ...
        "exec_cancel 停止；running 是命令仍在执行的正常状态，不是失败。安装、构建等长任务不要用 "
        "tail/head/grep 管道隐藏进度；多步骤命令要让前一步失败真实传播（POSIX 用 set -e，"
        "PowerShell 用 $ErrorActionPreference=\"Stop\"），不要用最后一步成功掩盖前面的失败。"
