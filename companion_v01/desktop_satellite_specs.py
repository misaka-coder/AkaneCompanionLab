...
from .computer_use.contracts import COMPUTER_USE_TOOL_SPEC
...
SYSTEM_PROCESS_SNAPSHOT_TOOL_SPEC = CapabilityToolSpec(
    ...
    description=(
        "查询绑定电脑全部进程中的 pid 与进程名。寻找应用时用 query 按进程名包含匹配（不区分大小写），"
        "分页用 offset/limit；hasMore=true 时继续查询，不能把当前页当成全部。应用名和 exe 名可能不同。"
        "不返回命令行、路径、用户或任何凭据；"
...
DESKTOP_SCREENSHOT_TOOL_SPEC = CapabilityToolSpec(
    capability_id="desktop_screenshot",
    display_name="Capture desktop screenshot",
    description=("按用户要求截取绑定电脑的主屏幕当前可见画面。返回可查看的图片及 gen_* 文件句柄；"
                 "需要发给用户时再用 send_file 交付该句柄。不是浏览器截图；没有在线设备或屏幕无法读取时明确失败。"),
    input_schema=_EMPTY_INPUT,
    output_schema={"type": "object", "properties": {
        "ok": {"type": "boolean"}, "width": {"type": "integer"},
        "height": {"type": "integer"}, "capturedAt": {"type": "integer"},
        "mimeType": {"type": "string"}, "imageBase64": {"type": "string"},
    }, "required": ["ok", "width", "height", "capturedAt", "mimeType", "imageBase64"],
        "additionalProperties": False},
    risk="medium", confirm="never", effects=("read_desktop_screen",),
    visible_in=("desktop", "qq"), idempotency="read_only", max_result_bytes=12 * 1024 * 1024,
)
...
DESKTOP_SATELLITE_TOOL_SPECS: tuple[CapabilityToolSpec, ...] = (
    DESKTOP_SCREENSHOT_TOOL_SPEC,
    COMPUTER_USE_TOOL_SPEC,
...
def desktop_satellite_spec(tool_id: str) -> CapabilityToolSpec | None:
    if tool_id == "browser_page_personal":
        from .personal_browser_contract import PERSONAL_BROWSER_SPEC
        return PERSONAL_BROWSER_SPEC
