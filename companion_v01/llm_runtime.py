...
from services.gemini_native_client import sanitize_openai_tools_for_gemini, gemini_thinking_config
...
from .engine_services.turn_context import build_desktop_screen_image_label
...
class _CompletionPayload(dict):
    """Wire fields plus request-local source positions, never sent as JSON keys."""

    persistent_message_indexes: tuple[int, ...] = ()


class ModelRequestCancelled(RuntimeError):
    """Host control flow: a revoked turn must not send another model request."""
...
class _ResponsesStreamAdapter:
    ...
    def __init__(self, raw_stream: Any):
        ...
        self.provider_content: list[dict[str, Any]] = []
    ...
    def __iter__(self):
        for event in self._raw_stream:
            ...
            if event_type == "response.output_item.done":
                item = self._get(event, "item")
                self._completed_items[self._tool_index(event, item)] = continuation.plain(item)
                self.provider_content = [self._completed_items[k] for k in sorted(self._completed_items)]
                if str(self._get(item, "type", "") or "") != "function_call":
                    continue
                index = self._tool_index(event, item)
            ...
            if event_type in {"response.completed", "response.incomplete"}:
                ...
                if output:
                    self.provider_content = continuation.plain(output)
...
class LLMRuntime:
        ...
        *,
        ...
        try:
            ...
            if native_tool_calls:
                ...
                extras: dict = {}
                continuation.collect_chat_extras(response, extras)
                state = continuation.capture(response, bundle=bundle, calls=native_tool_calls,
                                             extras=continuation.chat_extras(extras))
                if state:
                    parsed[continuation.RESULT_FIELD] = state
                metadata_status, metadata_present = _memory_metadata_truth(
                    parsed,
        ...
        *,
        ...
        try:
            ...
            for chunk in response:
                ...
                continuation.collect_chat_extras(chunk, provider_extras)
        ...
        if error:
            # An incomplete stream may omit the final signature or arguments.
            # Never execute the partial batch or turn it into a fresh decision.
            native_tool_calls = []
        ...
        if native_tool_calls:
            ...
            if reasoning_content:
                parsed[NATIVE_REASONING_CONTENT_FIELD] = reasoning_content
            state = continuation.capture(response, bundle=bundle, calls=native_tool_calls,
                                         extras=continuation.chat_extras(provider_extras))
    ...
    def _build_completion_kwargs(
        ...
        *,
        ...
        validation = isolate_tool_history(messages)
        messages = validation.messages
        if validation.issues:
            self._record_metric("tool_history_isolated", len(validation.issues))
            logger.warning("tool history isolated: issues=%s reasons=%s", len(validation.issues),
                           sorted({item["reason"] for item in validation.issues}))
        payload = _CompletionPayload({
            "model": bundle.model,
            "messages": messages,
        })
        payload.persistent_message_indexes = tuple(
            index for index, original in enumerate(validation.source_indexes)
            if original == current_message_index or original >= post_user_start
        )
        ...
        if normalized_tools and self._is_gemini_family_model(bundle) and not self._is_gemini_protocol(bundle):
            # Native generateContent converts once inside its adapter. Only
            # the OpenAI-compatible route needs this compatibility projection.
            normalized_tools = sanitize_openai_tools_for_gemini(normalized_tools)
        ...
        self._record_prompt_audit_if_enabled(
            ...
            post_user_turns=post_user_turns,
            user_prompt=user_prompt,
            user_image_count=sum(item.get("type") == "image_url" for item in image_items),
            prompt_audit_sections=prompt_audit_sections,
            stream=stream,
            json_mode=json_mode,
    ...
    def _normalize_post_user_turn_for_payload(
        ...
        *,
        ...
        if not self._is_anthropic_protocol(bundle):
            if role == "assistant":
                ...
                if tool_calls:
                    ...
                    return continuation.carry(turn, message, bundle=bundle)
            if role == "tool":
        ...
        if content and role in {"user", "assistant"}:
            return continuation.carry(turn, {"role": role, "content": content}, bundle=bundle)
        return None
    ...
    def _is_gemini_family_model(self, bundle: ModelBundle) -> bool:
        if self._is_gemini_protocol(bundle):
            return True
        model = str(getattr(bundle, "model", "") or "").strip().lower()
        return bool(re.search(r"(?:^|[^a-z0-9])gemini(?:[-_.]|$)", model))
    ...
    def _normalize_native_tools(self, value: Any) -> list[dict[str, Any]]:
        ...
        seen: set[str] = set()
        # Selection already owns exposure. Truncation here silently removes
        # published tools (for example load_skill and QQ delivery after plugins).
        for raw in value:
            if not isinstance(raw, dict):
                continue
    ...
    def _native_invocation_to_tool_call(
        ...
        *,
        ...
        result = {
            **({key: value for key, value in arguments.items() if not key.startswith("_tool_")}
               if isinstance(arguments, dict) else {}),
            "type": capability_id,
            TOOL_SOURCE_FIELD: source if source in {NATIVE_OPENAI, NATIVE_ANTHROPIC} else NATIVE_OPENAI,
            TOOL_MODEL_ARGUMENTS_FIELD: dict(arguments) if isinstance(arguments, dict) else {},
        }
    ...
    def _build_reasoning_control_kwargs(self, *, bundle: ModelBundle) -> dict[str, Any]:
        if self._is_gemini_protocol(bundle):
            thinking = gemini_thinking_config(bundle.model, self._configured_reasoning_effort(bundle))
            return {"thinking_config": thinking} if thinking else {}
        if self._is_responses_protocol(bundle):
    ...
    def _normalize_user_image_items(self, value: Any) -> list[dict[str, Any]]:
        ...
        for index, raw in enumerate(value[:5], 1):
            if not isinstance(raw, dict):
                continue
            url = str(raw.get("data_url") or raw.get("dataUrl") or raw.get("url") or "").strip()
            if not url.startswith("data:image/"):
                continue
            label = build_desktop_screen_image_label(raw, index)
            if label:
    ...
    @staticmethod
    def _dispatch_provider(create: Callable[..., Any], payload: dict[str, Any]) -> Any:
        # Check every actual SDK call, including compatibility and transport retries.
        # Production clients disable SDK retries (max_retries=0).
        if cancellation_requested():
            raise ModelRequestCancelled("turn_scope_revoked")
        return create(**payload)

    def _create_completion(self, *, bundle: ModelBundle, payload: dict[str, Any]) -> Any:
        # Direct callers and all compatibility retries share the same boundary.
        # Normal assembly already isolated history before observation/token counts.
        messages = [continuation.carry(m, m, bundle=bundle) if isinstance(m, dict) else m
                    for m in payload.get("messages") or []]
        payload = {**payload, "messages": isolate_tool_history(messages).messages}
        if self._is_responses_protocol(bundle):
            return self._create_responses_completion(bundle=bundle, payload=payload)
        if not self._is_gemini_protocol(bundle) and not self._is_anthropic_protocol(bundle):
            payload["messages"] = [continuation.chat_wire_message(m) for m in payload["messages"]]
        try:
            return self._dispatch_provider(bundle.client.chat.completions.create, payload)
        except TypeError as exc:
            stripped = self._without_prompt_cache_hints(payload)
            if stripped != payload:
                try:
                    return self._dispatch_provider(bundle.client.chat.completions.create, stripped)
    ...
    def _persistent_turn_messages_from_payload(
        ...
        *,
        ...
        if isinstance(payload, _CompletionPayload):
            persistent = [dict(payload["messages"][index]) for index in payload.persistent_message_indexes]
            return [continuation.public_message(m) for m in persistent]
        messages = [
            dict(message)
    ...
    def _observe_completion_request(
        ...
        *,
        ...
        if not callable(observer):
            return
        messages = [continuation.public_message(message) for message in list(payload.get("messages") or []) if isinstance(message, dict)]
        system_message = (
            dict(messages[0])
        ...
        if self._is_responses_protocol(bundle):
            wire_request = self._responses_payload_from_chat({**payload, "messages": messages})
    ...
    def _create_responses_completion(self, *, bundle: ModelBundle, payload: dict[str, Any]) -> Any:
        ...
        try:
            response = self._dispatch_provider(bundle.client.responses.create, request)
        except TypeError:
            stripped = self._without_prompt_cache_hints(request)
            if stripped == request:
                raise
            response = self._dispatch_provider(bundle.client.responses.create, stripped)
        except Exception as exc:
    ...
    def _responses_input_from_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        messages = isolate_tool_history(messages).messages
        items: list[dict[str, Any]] = []
        ...
        for message in messages:
            ...
            if role == "assistant" and isinstance(message.get("tool_calls"), list):
                saved_output = continuation.wire_data(message, "responses")
                if saved_output:
                    items.extend(saved_output)
                    continue
    ...
    def _adapt_responses_result(self, response: Any) -> Any:
        ...
        return SimpleNamespace(choices=[choice], usage=self._get_attr_or_key(response, "usage"), raw_response=response,
                               provider_content=continuation.plain(output))
