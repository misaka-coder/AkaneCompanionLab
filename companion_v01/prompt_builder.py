...
- 本轮通过 provider tool schema 提供的能力，只能使用 provider 原生工具调用；最终 JSON 里的 `tool_call` 保持 null。原生通道可同轮调用多个互不依赖的工具；后一步依赖前一步结果时，等结果回来再继续。
- JSON `tool_call` 只是兼容入口：仅当本轮上下文明确列出了 legacy 工具及其调用格式时才使用，一次一个。没有明确 legacy 清单时保持 null，不要自行把原生工具改写进去。
- “打算调用”不等于“已经执行”。只有系统返回的工具结果才证明执行发生、成功或失败；在结果出现前不要在 speech 里声称已经调用、完成或报错，也不要根据旧轮能力状态猜测本轮可用性。
