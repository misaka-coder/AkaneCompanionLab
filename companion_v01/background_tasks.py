...
class BackgroundTaskRunner:
    ...
    def close(self, *, timeout: float = 2.0) -> bool:
        if self._closed.is_set():
            with self._lock:
                return not any(thread.is_alive() for lane in self._lanes.values() for thread in lane.threads)
        self._closed.set()
        with self._lock:
    ...
    def wait_idle(self, *, lane: str | None = None, timeout: float = 2.0) -> bool:
        ...
        while True:
            with self._lock:
                ...
                if all(item.queue.unfinished_tasks == 0 for item in lanes):
                    return True
            for item in lanes:
                remaining = deadline - time.time()
                if remaining <= 0 or not item.wait_idle(timeout=remaining):
                    return False
            # A task can create another lane (e.g. thinking -> TTS) or enqueue
            # work into a lane already visited. Recheck the live set together.
