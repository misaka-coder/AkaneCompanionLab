...
from .tts_provider_selection import select_tts_provider
...
TOOL_USED_BY: dict[str, list[str]] = {
    ...
    "browser_page": ["agent", "desktop_pet", "qq_text"],
...
def build_local_capability_catalog(
    *,
    ...
    discover = getattr(getattr(engine, "plugin_capability_source", None), "list_services", None)
    try:
        services = discover("tts") if callable(discover) else []
        tts_service = next((item for item in services if item.get("version") == 1), {})
    except Exception:
        tts_service = {}
    for entry in entries:
        if entry.get("type") != "tts_provider":
            continue
        entry.update(source="plugin_service", executionMode="plugin",
            serviceProviderId=tts_service.get("selected_provider") or "",
            serviceStatus=tts_service.get("status") or "unavailable")
        if tts_service.get("status") != "available":
            entry.update(status="unavailable", reason=tts_service.get("reason") or "service_unavailable")
        elif entry.get("id") == "provider.tts.edge":
            entry.update(enabled=True, status="ready", reason="", requiresConfirmation=True, risk="medium")
    ...
    resolutions = _build_voice_provider_resolutions(entries, character_voice=character_voice, tts_service=tts_service)
...
def _build_voice_provider_resolutions(
    ...
    *,
    character_voice: Mapping[str, Any] | None = None,
    tts_service: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    selection = select_tts_provider(character_voice, allow_unknown=tts_service is not None)
    requested_tts = selection.provider_id
    selected_entry = by_id.get(requested_tts)
    ready = not selection.reason and _is_provider_ready(selected_entry)
    unavailable_reason = "requested_provider_not_ready"
    if tts_service is not None:
        service_ready = tts_service.get("status") == "available"
        if tts_service.get("selected_provider") != "akane.tts":
            # Third-party voice support is decided by that service's contract,
            # not by the legacy Edge/GPT configuration catalogue.
            ready = not selection.reason and service_ready
        else:
            ready = ready and service_ready
        if not service_ready:
            unavailable_reason = str(tts_service.get("reason") or "service_unavailable")
    tts_resolution = {
        "capabilityId": "voice.tts.character",
        "strategy": "selected_provider",
        "status": "disabled" if selection.reason == "text_only_requested" else "ready" if ready else "unavailable",
        "reason": selection.reason or ("" if ready else unavailable_reason),
        "requestSource": "character_pack" if character_voice else "default",
        "requestedProviderId": requested_tts,
        "requestedProviderName": _provider_name(selected_entry, requested_tts),
        "activeProviderId": requested_tts if ready else "",
        "activeProviderName": _provider_name(selected_entry, requested_tts) if ready else "",
        "fallbackProviderId": "",
        "voiceProfileId": selection.voice_profile_id,
        "candidates": [_candidate_summary(requested_tts, selected_entry)] if requested_tts else [],
        **({"serviceProviderId": tts_service.get("selected_provider") or "",
            "serviceStatus": tts_service.get("status") or "unavailable"} if tts_service is not None else {}),
    }
...
def _resolve_first_ready_provider(
    *,
    ...
    for candidate_id in unique_candidates:
        if _is_provider_ready(entries_by_id.get(candidate_id), accepted_statuses=accepted_statuses):
    ...
    status = (
        "ready"
        if active_provider_id and active_provider_id == requested_provider_id
        else "degraded"
    )
