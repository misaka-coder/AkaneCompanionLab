...
class VoicePlaybackDeliveryChannel:
    ...
    async def wait_activity(self) -> None:
        ...
        with self._guard:
            ...
            if self._outbound or self._response_terminal_unlocked():
                return
            activity.clear()
        await activity.wait()
...
class AkaneVoicePlaybackCommandExecutor:
    """Offer ``enqueue_playback`` to its negotiated live client."""
    ...
    def execute(
        ...
        offered = channel.offer(command_record, snapshot_record)
    ...
    def _context(
        ...
        if command.get("command_kind") != "enqueue_playback" or not isinstance(payload, Mapping):
            return "voice_playback_command_invalid"
        response_id = str(payload.get("response_id") or "")
        responses = snapshot_record.get("responses")
        response = responses.get(response_id) if isinstance(responses, Mapping) else None
