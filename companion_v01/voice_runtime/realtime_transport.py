...
async def _send_response_presentation(websocket: Any, channel: Any, version: int, voice_turn_id: str) -> None:
    take = getattr(channel, "take_presentation", None)
    presentation = take() if callable(take) else None
    if presentation is not None:
        await websocket.send_json(
            {
                "type": "server.response.presentation",
...
class VoiceRealtimeWebSocketSession:
    ...
    async def _handle_finalize_completion(self) -> None:
        ...
        if response_status and response_status not in {"started", "completed"}:
    ...
    def _arm_delivery_wait(self) -> None:
        if self.delivery_channel is None or self.delivery_task is not None or self.terminal:
            return
        interaction_committed = getattr(self.delivery_channel, "interaction_committed", None)
        if not self.final_sent and callable(interaction_committed) and interaction_committed():
            return
    ...
    async def _handle_delivery_activity(self) -> None:
        if self.delivery_channel is None:
            return
        await _send_response_presentation(
            self.websocket,
            self.delivery_channel,
            VOICE_REALTIME_PROTOCOL_VERSION,
            self.open_request.voice_turn_id if self.open_request else "",
        )
        interaction_committed = getattr(self.delivery_channel, "interaction_committed", None)
        if callable(interaction_committed) and interaction_committed():
            if not self.final_sent:
                return
            await self.websocket.send_json(
                {
                    "type": "server.turn.interaction",
                    "protocol_version": VOICE_REALTIME_PROTOCOL_VERSION,
                    "voice_turn_id": self.open_request.voice_turn_id if self.open_request else "",
            ...
            self.delivery_channel.close(reason="voice_interaction_committed")
            self.terminal = True
            self._observe_once(ok=True)
            await self.websocket.close(code=1000, reason="voice_interaction_committed")
            return
...
class VoiceRealtimeCallWebSocketSession:
    ...
    def _arm_delivery_wait(self, voice_turn_id: str) -> None:
        ...
        if channel is None or self.terminal:
            return
        interaction_committed = getattr(channel, "interaction_committed", None)
        if (
            self.current_request is not None
            and self.current_request.voice_turn_id == voice_turn_id
            and callable(interaction_committed)
            and interaction_committed()
        ):
            return
    ...
    async def _handle_delivery_activity(self, voice_turn_id: str) -> None:
        ...
        interaction_committed = getattr(channel, "interaction_committed", None)
        if callable(interaction_committed) and interaction_committed():
            # Always deliver ASR final first, even if the semantic worker wins
            # the race with the finalize task on this event loop.
            if self.current_request is not None and self.current_request.voice_turn_id == voice_turn_id:
                return
            await self.websocket.send_json(
                {
                    "type": "server.turn.interaction",
            ...
            self._close_delivery_channel(voice_turn_id, reason="voice_interaction_committed")
            return
    ...
    async def _finish_delivered_response(
        ...
        await _send_response_presentation(
            self.websocket,
            self.delivery_channels[voice_turn_id],
            VOICE_REALTIME_CALL_PROTOCOL_VERSION,
            voice_turn_id,
        )
        event_type = "server.response.completed" if outcome.response_state == "completed" else "server.response.failed"
    ...
    async def _send_provider_updates(self, result: Any) -> bool:
        ...
        for revision in tuple(getattr(update, "revisions", ()) or ()):
            ...
            if message_type == "server.turn.final":
                input_state = str(getattr(result, "input_state", "") or "")
                if input_state:
                    payload["input_state"] = input_state
                    payload["disposition"] = str(getattr(result, "disposition", "") or "")
