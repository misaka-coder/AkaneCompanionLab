...
from .playback_context import render_unsettled_playback_context
...
class _GenerationJob:
    ...
    presentation_sink: Callable[[str, Mapping[str, Any]], None] | None = None
...
class AkaneThinkingAgentCommandExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self._presentation_sinks: dict[str, Callable[[str, Mapping[str, Any]], None]] = {}
    ...
    def register_turn(
        ...
        *,
        ...
        with self._guard:
            ...
            if presentation_sink is not None:
                self._presentation_sinks[normalized_turn_id] = presentation_sink
    ...
    def start_ready_generations(
        ...
        *,
        ...
        if not ready:
            ...
            with self._guard:
                already_running = existing is not None and existing.response_id in self._running_response_ids
            if already_running:
                return VoiceThinkingStartResult(status="started", response_id=str(existing.response_id))
    ...
    def close(self) -> None:
        with self._guard:
            ...
            self._presentation_sinks.clear()
    ...
    def _execute_or_recover(
        ...
        *,
        ...
        job = _GenerationJob(
            ...
            presentation_sink=self._presentation_sinks.get(voice_turn_id),
        )
        with self._guard:
    ...
    def _run_generation(
        ...
        try:
            if not cancelled:
                playback_context = render_unsettled_playback_context(
                    host.snapshot,
                    self.text_artifacts,
                    exclude_response_id=job.response_id,
                )
                playback_kwargs = {"playback_context": playback_context} if playback_context else {}
            ...
            for stream_event in iterator or ():
                ...
                if event_type == "final":
                    ...
                    if isinstance(final_payload, Mapping) and bool(final_payload.get("_transient_final_failure")):
                        ...
                        break
                    # Stage before the terminal observation can wake transport.
                    # The channel exposes it only after VoiceCore accepts final.
                    if job.presentation_sink is not None and isinstance(final_payload, Mapping):
                        job.presentation_sink(job.response_id, final_payload)
        ...
        with self._guard:
            ...
            if job.commitment != "speculative":
                ...
                self._presentation_sinks.pop(job.voice_turn_id, None)
