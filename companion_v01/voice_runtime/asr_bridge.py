...
class VoiceASRSessionBridge:
    ...
    def __init__(
        ...
        *,
        ...
        supersede_prior_responses: bool = False,
    ) -> None:
        self.host = host
        ...
        self.supersede_prior_responses = bool(supersede_prior_responses)
    ...
    def input_state(self) -> str:
        ...
        return str(getattr(getattr(turn, "state", None), "value", "") or "")
    ...
    def _commit_final(self, revision: ASRTranscriptRevision) -> VoiceASRBridgeResult:
        ...
        dispatches = [dispatch]
        if (
            dispatch.status in {"accepted", "duplicate"}
            and self.supersede_prior_responses
            and self.committed_disposition == "message"
            and not any(
                attempt.voice_turn_id == self.voice_turn_id
                for attempt in self.host.snapshot.interruption_attempts.values()
            )
        ):
            # Ordinary dialogue replaces obsolete responses only after commit.
            # Overlapping speech stays under the semantic controller's policy.
            for response in tuple(self.host.snapshot.responses.values()):
                if (
                    response.voice_turn_id == self.voice_turn_id
                    or response.state.value in {"completed", "cancelling", "cancelled", "failed", "discarded"}
                ):
                    continue
                cancel = self._make_event(
                    "voice.response.cancel_requested",
                    provider_receipt_id=self._receipt(f"superseded:{response.response_id}"),
                    voice_turn_id=response.voice_turn_id,
                    response_id=response.response_id,
                    turn_revision=response.source_turn_revision,
                    response_generation=response.response_generation,
                    payload={"reason": "new_voice_turn_committed"},
                )
                if cancel is None:
                    return self._failed("final", "voice_event_build_failed")
                dispatches.append(self.host.accept_event(cancel))
        return self._from_dispatches("final", tuple(dispatches))
