...
class AkaneVoiceSemanticPulseCommandExecutor:
    ...
    def _drive_available_commands(host: AkaneVoiceRuntimeHost) -> None:
        for _ in range(8):
            ...
            if not before.pending_commands:
                return
            result = host.drive_once()
            # A stop/duck awaiting client ACK must not stall the separate
            # response-created -> generation-started command chain.
            if result.status == "failed" or host.snapshot is before:
                return
