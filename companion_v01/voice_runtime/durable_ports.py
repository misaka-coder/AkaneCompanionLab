...
from .host import VoiceHostPortResult
...
class SqliteVoiceRuntimeJournal:
            ...
            try:
                ...
                try:
                    ...
                    existing = connection.execute(
                        "SELECT event_sha256 FROM voice_events WHERE event_id = ?",
                        (event.event_id,),
                    ).fetchone()
                    ...
                    connection.execute(
                        ...
                        """,
                        (ordinal, event.event_id, digest, canonical),
                    )
                    connection.execute(
    ...
    def _ensure_schema(connection: sqlite3.Connection) -> None:
        ...
        meta_rows = connection.execute(
            "SELECT singleton_id, schema_version FROM voice_journal_meta"
        ).fetchall()
        if not meta_rows:
            connection.execute(
        ...
        head_rows = connection.execute(
            "SELECT singleton_id, record_count FROM voice_journal_head"
        ).fetchall()
        if not head_rows:
            connection.execute(
                "INSERT INTO voice_journal_head (singleton_id, record_count) VALUES (1, 0)"
            )
    ...
    def _validated_head(connection: sqlite3.Connection) -> int:
        ...
        if (
            ...
            or minimum_ordinal != expected_minimum
            or maximum_ordinal != record_count
        ):
...
def _conversation_storage_root(
    *,
    ...
    try:
        ...
        namespace = sha256(
            f"{conversation_id}\0{conversation_generation}".encode("utf-8")
        ).hexdigest()
