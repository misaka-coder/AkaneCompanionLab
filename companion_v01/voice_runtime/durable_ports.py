    ...
    def _validated_command_receipts(connection: sqlite3.Connection) -> None:
        ...
        for row in rows:
            ...
            if (
                ...
                raise _JournalStorageError("voice_command_receipt_unreadable")
            normalized_observations = [voice_event_to_dict(observation) for observation in observations]
            canonical_observations = [_canonical_json(observation) for observation in normalized_observations]
            if (
                _record_set_digest(canonical_observations) != expected_observation_digest
                or _canonical_json_value(normalized_observations)
                != observations_json
            ):
                raise _JournalStorageError("voice_command_receipt_unreadable")
...
class FileVoiceAudioArtifactPort:
    ...
    def put_audio(
        ...
        *,
        ...
        if origin is not None and not _valid_audio_origin(origin):
            return VoiceAudioArtifactResult.failed("voice_audio_artifact_origin_invalid")
        ...
        with self._guard:
            if path.exists():
                ...
                if (existing.audio == audio and existing.media_type == normalized_media_type
                        and existing.origin == dict(origin or {})):
    ...
    def _read_record(
        *,
        ...
        if (
            ...
            or set(metadata) not in (required, required | {"origin"})
            or ("origin" in metadata and not _valid_audio_origin(metadata["origin"]))
            or match is None
        ...
        return VoiceAudioArtifactReadResult(
            ...
            origin=dict(metadata.get("origin") or {}),
        )


def _valid_audio_origin(origin):
    keys = {"plugin_id", "generation_id", "capability_id", "invocation_id"}
    # Playback-only TTS has provenance but no durable workbench artifact.
    return (isinstance(origin, Mapping) and keys <= set(origin) <= keys | {"generated_handle"}
        and all(isinstance(value, str) and re.fullmatch(r"[A-Za-z0-9_:.-]{1,256}", value)
