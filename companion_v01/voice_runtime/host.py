...
class VoiceCommandExecutionResult:
    ...
    @classmethod
    def running(cls, reason: str) -> VoiceCommandExecutionResult:
        """An effect is still running locally; retain its durable executing receipt."""
        return cls(
            status="deferred",
            reason=reason,
            retryable=True,
...
class AkaneVoiceRuntimeHost:
    ...
    def _validate_command_execution_result(
        *,
        ...
        if result.in_progress and (result.status != "deferred" or result.outcome_known or result.observations):
            return VoiceCommandExecutionResult.unknown("invalid_command_in_progress_result")
        ...
        for command_id in command_ids:
            ...
            if execution.outcome_known:
                ...
                if not released.ok:
                    ...
                    break
            elif not execution.in_progress:
                break
        ...
        for index, receipt in enumerate(records):
            ...
            if receipt.phase == "executing":
                ...
                if recovery.in_progress:
                    # A live asynchronous effect owns this receipt. It must not
                    # be released/retried or block unrelated control commands.
                    continue
