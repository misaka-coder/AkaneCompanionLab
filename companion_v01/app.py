...
if config.RUNTIME_MEMORY_TRACING_ENABLED:
    tracemalloc.start()
...
# A freshly installed local-default keeps the pre-profile loopback contract
# until an admin token is configured. Named instances and token-backed local
# profiles continue through the immutable manifest path.
selected_instance_id = str(getattr(config, "AKANE_INSTANCE_ID", "") or "").strip()
if selected_instance_id == "local-default" and not str(getattr(config, "AKANE_ADMIN_TOKEN", "") or "").strip():
...
for qq_bot_runtime in bot_registry.values():
    ...
    qq_route_kwargs = {
        ...
        # Both URL aliases register the same queue source handler. Keep its
        # plugin receipt owner available whichever alias is registered last.
        "plugin_turn_router": qq_bot_runtime.plugin_agent_event_router,
    }
    app.include_router(
        build_qq_router(
            **qq_route_kwargs,
