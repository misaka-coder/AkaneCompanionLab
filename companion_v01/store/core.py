...
class MemoryStore:
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS chat_messages (
                    ...
                    prompt_envelope_text TEXT NOT NULL DEFAULT '',
            ...
            self._ensure_column(
                conn=conn,
                table_name="chat_messages",
                column_name="prompt_envelope_text",
                column_definition="TEXT NOT NULL DEFAULT ''",
            )
            self._ensure_column(
                conn=conn,
    ...
    def upsert_message_prompt_envelope(
        self,
        *,
        source_id: str,
        profile_user_id: str,
        ...
        """Persist the exact provider-visible user turn without changing memory facts.

        The envelope is a private field on that same raw message and is only
        used to rebuild an append-only provider prefix.  It is intentionally
        separate from ``memory_metadata`` so retrieval and vector indexing do
        not treat transport/visual hints as user facts.
        """

        sid = str(source_id or "").strip()
        profile = str(profile_user_id or "").strip()
        session = str(session_id or "").strip()
        character = normalize_character_pack_id(character_pack_id)
        text = str(prompt_text or "").strip()
        if not sid or not profile or not session or not text:
            return {"ok": False, "status": "invalid", "reason": "required_field_missing"}
        with self._connect() as conn:
            owner = conn.execute(
                """
                SELECT profile_user_id, session_id, character_pack_id
                FROM chat_messages
                WHERE source_id = ?
                """,
                (sid,),
            ...
            if owner is None:
                return {"ok": False, "status": "not_found", "reason": "message_not_found"}
            actual_owner = (
                str(owner["profile_user_id"] or ""),
                str(owner["session_id"] or ""),
                normalize_character_pack_id(owner["character_pack_id"]),
            )
            if actual_owner != (profile, session, character):
                return {"ok": False, "status": "namespace_mismatch", "reason": "owner_mismatch"}
            conn.execute(
                "UPDATE chat_messages SET prompt_envelope_text = ? WHERE source_id = ?",
                (text, sid),
            )
        return {"ok": True, "status": "stored", "reason": ""}

    def get_message_prompt_envelopes(
        self,
        source_ids: list[str],
        *,
        profile_user_id: str,
        ...
        ids = list(dict.fromkeys(str(source_id or "").strip() for source_id in source_ids))
        ids = [source_id for source_id in ids if source_id]
        if not ids:
            return {}
        ...
        with self._connect() as conn:
            rows = conn.execute(
                f"""
                SELECT source_id, prompt_envelope_text
                FROM chat_messages
                WHERE profile_user_id = ?
                  AND session_id = ?
                  AND character_pack_id = ?
        ...
        return {
            str(row["source_id"] or ""): str(row["prompt_envelope_text"] or "")
            for row in rows
            if str(row["source_id"] or "").strip() and str(row["prompt_envelope_text"] or "").strip()
        }

    def delete_message_prompt_envelope(
        self,
        *,
        source_id: str,
        profile_user_id: str,
    ...
    def prune_message_prompt_envelopes(
        self,
        *,
        profile_user_id: str,
        session_id: str,
        character_pack_id: str = "",
        keep_source_ids: list[str] | None = None,
    ) -> int:
        keep = list(dict.fromkeys(str(source_id or "").strip() for source_id in keep_source_ids or []))
        keep = [source_id for source_id in keep if source_id]
        params: list[Any] = [
            str(profile_user_id or "").strip(),
            str(session_id or "").strip(),
            normalize_character_pack_id(character_pack_id),
        ...
        if keep:
            ...
            keep_clause = f" AND source_id NOT IN ({placeholders})"
            params.extend(keep)
        with self._connect() as conn:
            cursor = conn.execute(
                f"""
    ...
    def mark_messages_summarized(self, source_ids: list[str], summary_id: str) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                f"""
                UPDATE chat_messages
                SET is_summarized = 1, summary_id = ?, prompt_envelope_text = ''
