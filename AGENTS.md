...
- UI 不直接调后端/Tauri。
- 后端不执行桌面端动作。
- 密钥、宿主内部路径（数据库/缓存/run log 物理位置）不进入日志、snapshot 和稳定提示；模型完成任务所需的操作路径可以出现在工具结果与会话历史中。不得用脱敏标记（如 `[local_path]`）破坏命令和路径的可执行语义。
...
- 包不是必须回填；只有明显降低 Akane 迭代复杂度、减少宿主自有逻辑、或删除一条旧路径时才应用回 Akane。
- 如果包回填后的用户效果和原逻辑差不多，优先删除旧逻辑、冗余字段、冗余状态和重复提示；不要让两个入口长期一起维护。
- 没有真实可调用能力的 future-only 占位提示、占位 UI、占位状态不要进入运行时或 prompt。需要预留方向时写进 docs，不要塞进用户可见/模型可见链路。
