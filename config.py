...
class Settings(BaseSettings):
    # Allocation tracing is diagnostic work; keep it off the normal turn path.
    RUNTIME_MEMORY_TRACING_ENABLED: bool = False
    # === 运行模式 & 人设 ===
    RUN_MODE: str = "CLOUD"
    ...
    # 插件运行依赖供应：本地 wheelhouse 或显式允许的包索引，二选一。
    AKANE_PLUGIN_DEPENDENCY_WHEELHOUSE: str = ""
    AKANE_PLUGIN_DEPENDENCY_INDEX_URL: str = ""
    # 仅模型结果预览预算；完整 JSON 通过现有材料读取链续读。
    AKANE_PLUGIN_RESULT_PREVIEW_CHARS: int = Field(default=16_000, ge=1)
    # 启动级执行资源策略 JSON；额度不属于业务权限或工具 schema。
    AKANE_EXECUTION_RESOURCE_POLICY: str = "{}"
    AKANE_EXECUTION_POLICIES: str = "[]"
    ...
    # 显式注入给受信任执行器的凭据环境变量引用（只配置名称，不配置值）。
    # 值仍由宿主环境持有；执行输出与持久日志会按实际值精确遮蔽。
    EXECUTION_CREDENTIAL_ENV_NAMES: str = ""
    ...
    # Migration input only: when no saved toolExposure.defaultMode exists,
    # True maps host/plugin tools to on_demand; False preserves resident defaults.
    # Both values use the unified load/invoke engine; no retired G1 branch remains.
    ENABLE_PROGRESSIVE_CAPABILITY_DISCOVERY: bool = False
    ...
    # OneBot 附件首次物化失败时，私有重试定位信息最多保留多久（秒）
    QQ_ATTACHMENT_SOURCE_LOCATOR_TTL_SECONDS: int = 7 * 24 * 60 * 60
    # QQ 文本消息附件最大读取字节数
...
def _apply_settings(s: Settings) -> None:
    global RUNTIME_MEMORY_TRACING_ENABLED
    RUNTIME_MEMORY_TRACING_ENABLED = bool(s.RUNTIME_MEMORY_TRACING_ENABLED)
    ...
    global ENABLE_PROGRESSIVE_CAPABILITY_DISCOVERY
    ...
    global \
        QQ_ATTACHMENT_DOWNLOAD_TIMEOUT, \
        QQ_ATTACHMENT_MAX_BYTES, \
    ...
    global AKANE_PLUGIN_DEPENDENCY_WHEELHOUSE
    global AKANE_PLUGIN_DEPENDENCY_INDEX_URL
    global AKANE_PLUGIN_RESULT_PREVIEW_CHARS
    global AKANE_EXECUTION_RESOURCE_POLICY
    global AKANE_EXECUTION_POLICIES
    global \
        EMBEDDING_PROVIDER, \
    ...
    ENABLE_PROGRESSIVE_CAPABILITY_DISCOVERY = bool(s.ENABLE_PROGRESSIVE_CAPABILITY_DISCOVERY)
    ...
    QQ_ATTACHMENT_SOURCE_LOCATOR_TTL_SECONDS = max(0, int(s.QQ_ATTACHMENT_SOURCE_LOCATOR_TTL_SECONDS))
    ...
    AKANE_PLUGIN_DEPENDENCY_WHEELHOUSE = str(s.AKANE_PLUGIN_DEPENDENCY_WHEELHOUSE or "").strip()
    AKANE_PLUGIN_DEPENDENCY_INDEX_URL = str(s.AKANE_PLUGIN_DEPENDENCY_INDEX_URL or "").strip()
    AKANE_PLUGIN_RESULT_PREVIEW_CHARS = s.AKANE_PLUGIN_RESULT_PREVIEW_CHARS
    AKANE_EXECUTION_RESOURCE_POLICY = s.AKANE_EXECUTION_RESOURCE_POLICY
    AKANE_EXECUTION_POLICIES = s.AKANE_EXECUTION_POLICIES
