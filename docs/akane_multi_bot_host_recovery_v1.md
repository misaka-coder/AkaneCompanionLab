...
- next 桌宠 `PetState` 与 launch binding 已加入 `hostId + boundBotId`，旧 `instanceId` 保留为 Host 兼容别名；旧状态会安全回填，非法 Bot id 与部署环境强绑定冲突会 fail-closed。
- 本地 Slice 5B 已完成桌宠 HTTP dispatch：`app.py` 对每个 Registry runtime 复用注册 `/api/bots/{bot_id}` 下的 think/session/vision/TTS/ASR/workspace/capabilities/control-center/`pet` 等同一组 Router，旧无前缀地址只作为默认 Bot 兼容别名，不存在 personal/finance 两套实现。
- Host 公开 `GET /api/bots` 只返回安全 Bot id、显示名与生命周期状态；控制中心唯一实现 `control-center-lab.html` 已加入当前 Bot 选择器，选择通过既有 settings-command bridge 写入 `boundBotId`。主窗口、侧面板、工作区和控制中心数据源均使用共享 `bot-routing.js` 生成 Bot-scoped URL。
- 每 Bot 角色包、petdesk 资源、TTS 临时音频与 `user-assets` 使用相同 Bot URL namespace；Rust 管理代理只允许访问当前 `boundBotId` 的 management 子路径，拒绝跨 Bot 或借管理代理调用普通 `/think`。
...
- 当前 personal 能力目录的真实状态：附件读取、文件生成/发送与上述四项 Satellite 工具为 ready；Edge TTS 为 ready；GPT-SoVITS 为 `missing_config / provider_endpoint_missing`；faster-whisper ASR 缺执行器并降级到文本输入；图像生成工具未挂载。尚未完成真实桌宠消息、GPT-SoVITS、文件传输、模型驱动 Satellite 调用或识图在桌宠/QQ UI 中的用户表现验收，不能把“路由存在”写成“完整本地能力已恢复”。
- 云端视觉服务此前忽略 Bot 的 `responses` 协议并固定调用 `/chat/completions`，导致 PinAI 文本可用但桌宠/附件/QQ 图片观察长期停在 pending。共享 `VisionObservationService` 已改为按 runtime 协议生成 Responses `instructions + input_text + input_image` 请求，并以 release `3bb3833` 上线；102 项视觉/运行时/路由回归与远端聚焦测试通过。真实图片 smoke 在约 21 秒返回 `ready`，能识别银白长发、猫耳、紫色服装、蝴蝶结与尾巴等具体细节，独立测试会话随后已清理。该结果证明统一视觉分析链可用，但 QQ 图片投递与桌宠实际 UI 表现仍需分别验收。
- 缓存真实性 repair pass 已在本地完成。PinAI Responses 的重复 12,014-token 实测中，`24h` 连续两次均为 `cached_tokens=0`；同一请求改为 `in-memory` 后第二次复用 11,008 tokens（约 91.6%）。因此 Host 仍接受统一的 `24h` 配置语义，但向 PinAI Responses 发包时兼容转换为真实生效的 `in-memory`，并记录 `prompt_cache_retention_compat` 指标；设置目录也明确 PinAI 应使用 `in-memory`。
- 同一 provider cache key 下只改变消息尾部时，第二轮复用 6,912 / 7,217 input tokens（约 95.8%），证明 MemCore 尾部追加本身不会破坏稳定前缀；工具集合从 13 变为 14 时当轮缓存归零，相同 14 工具的下一轮复用 6,656 / 7,454 tokens（约 89.3%），证明 PinAI Responses 将完整 tools 集合纳入前缀匹配，不能让运行时健康状态反复改写 schema。
- 13→14 的唯一变化是 `web_search`。Host 重启或 readiness TTL 过期后的首轮探针会短暂报告 `checking`，旧逻辑因此把它从 native schema 移除；探针完成后又恢复。现在 capability selection 显式分离稳定 `schema_tool_names` 与当轮 ready `tool_names`：配置/插件挂载决定 schema，`checking/online/offline` 只决定执行状态。临时不可用工具仍保留 schema，真实调用返回结构化 unavailable，不假成功，也不再破坏连续轮次的缓存前缀。该规则适用于全部配置能力，不是 `web_search` 或某个 Bot 的特判。
- Responses Chat/Aux reasoning 已改为读取每个 `BotSettingsView`，不再直接读取进程全局 `config`。Host 默认 Chat 可设为 `high`；personal 继承该值，finance 的独立 model-service 可保留空的 Chat reasoning，从而不再误继承 personal 的 `max/high`。模型服务配置解析、公开安全快照和 runtime overlay 均已覆盖该字段。
- repair pass 同时修复 Responses 终帧 usage 被流结束逻辑重复累计的问题，并补充 `cache_write_tokens` 解析；prompt cache key 只按稳定路由身份分桶，不再混入瞬时 tool/disclosure hash。provider 仍按完整请求内容校验真实前缀，不会因为 key 稳定而错误复用不同提示词。
- 本地验证通过：缓存/runtime/model-service/readiness 聚焦 82 项；tool readiness、VN/native web/plugin bridge 组合 119 项；backend routes、BotRuntime、Host bootstrap、multi-Bot desktop、MemCore、QQ voice 组合 162 项；语法编译与 `git diff --check` 通过。真实云端个人消息与金融推送缓存仍必须在该 release 上线后分别验收，不能以合成请求代替表现验收。
