...
| N27 | Effectful invocation gets no ACK after send | Return `execution_unknown`, including disconnect after sending starts; missing ACK is not proof of no execution. Do not silently retry or mark success. Only an invocation blocked before sending may return `unavailable_before_dispatch`. |
