...
- **约束**：同一决策可并行执行互不依赖的工具；有依赖的步骤等待真实结果后再进入下一轮。默认 `TOOL_ROUND_HARD_LIMIT=0`，合法工具调用持续执行，直到模型交付、用户停止或真实错误结束；只有显式配置正数才启用按实际工具批次计算的可选硬上限。带名称的工具调用即使参数错误、未知或被拒，也要成为普通结构化工具结果；只有 Provider 连续没有形成可表示决策时才使用 `CHAT_MODEL_DECISION_MAX_ATTEMPTS` 有界重试，且不消耗真实工具预算、不改变工具权限。不存在按工具家族扩容的软预算，也不存在宿主猜测的“终稿阶段”：合法工具调用就执行并继续，合法交付就返回，格式错误就带明确反馈重新决策。
- **预算边界**：仅在显式启用有限硬上限时，到达上限的最后一批工具才先真实执行、记录并把结果喂回，随后关闭新工具并要求模型诚实交付。`TOOL_ROUND_WARNING_REMAINING` 也只对有限上限生效，在接近上限时解释一次 MemCore 结算与续作记录的原因，不改变工具权限；需要跨用户回合续做的编程任务应留下真实、可检查的项目记录，不伪造尚未产生的卡片/source ID。
...
4. **控制中心已切到真实数据 V2**。`control-center-lab.html` 直接加载 `src/control-center-v2/index.js`；断线时必须显示诚实空态/失败态，不得回退到 `mock-data.js`。`src/control-center-lab.js` 只是陈旧 import 的薄适配器，不得重新长出业务逻辑。
