...
### 2026-09-05：模型输入格式归一化

87 群两次“未交付”的真实错误是 Gemini 上游拒绝 `image/gif`（HTTP 500，`convert_request_failed`），不是角色输出不遵守 JSON。原请求重试仍携带 GIF，因此继续失败。

`model_image_inputs.py` 在 provider 组装边界为 GIF 生成 PNG 首帧，并在紧邻图片的文本块标注只提供静态首帧、不包含动画过程。当前输入、MemCore 物化后的历史图片、工具图片和后台视觉观察器复用同一转换。正常 PNG/JPEG/WebP 不变，不按角色或域名分支，不更改原附件、资源句柄、稳定系统提示词或工具 schema。格式错误或转换超限明确返回图片不可用提示，不将无效 GIF 反复发往上游。

这不是动画理解：完整 GIF 保留供后续工具读取，当前模型视图只保证首帧。转换限制为 8 MiB 输入/输出、16M 像素；不新建持久化派生文件或缓存服务。测试覆盖真实双帧 GIF、错误 MIME、损坏/超限图、当前/历史/工具载荷及观察器 Chat/Responses 两条协议。

已部署 `1ad7b8b`。本地两组回归 244 + 62 项通过，云端相关回归 156 项通过。认证 webhook 验收 `GIF_INPUT_SMOKE` 引用了原失败请求所引用的真实图片消息 `611398976`（原失败请求 `1443232329`）；不是另造一张简单图片代替原样本。测试输入被当时运行中的同主体回合接纳为 steer，附件物化为 1 张，随后 Gemini 请求 HTTP 200，群消息 `564836449` 正常包含图片描述，未发送失败兜底。角色仍为 `cecilia`，两 bot 自检 connected；没有 QQ 客户端截图验收。

同次 steer 还观察到 `projection_immutable_conflict` 的投影审计告警，模型请求和图片回复仍成功；该独立问题未在 GIF 格式修复中处理，不据此宣称全部上下文/中途插话链路均已验收。

1. 原生多模态模型应直接看到当前用户上传的图片，不再只看到另一个模型生成的文字摘要。
2. `VisionObservationService` 不删除，降级为后台观察器：负责摘要、OCR、可见细节、缓存与非多模态模型降级。
3. 当前聊天模型、视觉观察器和未来图像生成器都只通过受管 asset/material handle 访问图片；base64 和原文件内容不进入 memcore，存储位置（`storage_relpath` 等宿主内部字段）也不进入模型可见历史。
