...
- **5 client-handled actions** provide real local behavior (expand/collapse UI sections) with `refresh:false` and no backend/Tauri boundary.
...
These fields are verified in scenario 1 (production-shaped snapshot) and scenario 2 (partial degradation) of the runtime probe. The smoke test also confirms all 84 action IDs in `CONTROL_CENTER_ACTIONS` are classified as bridged, client-handled, or deferred — no unclassified action IDs exist, and duplicate page surfaces must stay in the same status category.
...
|--------|-------|----------|
| **Bridged** | 48 unique action IDs / 57 surface entries | Real settings command, Tauri invoke, Tauri window API, or dedicated backend route |
| **Client-handled** | 5 | Local UI toggle, `refresh:false`, no backend/Tauri boundary |
...
`music.playWorkspaceRecommendation` is bridged in this batch as a client-only settings command. `abilities.provider.config.save` and `abilities.provider.healthCheck` are bridged to dedicated provider routes, while `abilities.provider.config.open` is client-handled only. `voice.records.clear`, `voice.queue.clear`, and `advanced.logs.clear` remain deferred because `main.js` has no corresponding settings command handler.
