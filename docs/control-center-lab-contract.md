...
These fields are verified in scenario 1 (production-shaped snapshot) and scenario 2 (partial degradation) of the runtime probe. The smoke test also confirms all 91 action IDs in `CONTROL_CENTER_ACTIONS` are classified as bridged, client-handled, or deferred — no unclassified action IDs exist, and duplicate page surfaces must stay in the same status category.
...
|--------|-------|----------|
| **Bridged** | 54 unique action IDs / 63 surface entries | Real settings command, Tauri invoke, Tauri window API, or dedicated backend route |
...
`music.playWorkspaceRecommendation` is bridged in this batch as a client-only settings command. `abilities.provider.config.save`, `abilities.provider.healthCheck`, `abilities.provider.ttsTest`, and `abilities.provider.voiceProfile.save` are bridged to dedicated provider routes, while `abilities.provider.config.open` is client-handled only. `abilities.provider.ttsTest` returns only a bounded short audio sample for local playback and does not persist voice profile/model fields. `abilities.provider.voiceProfile.save` persists profile-scoped GPT-SoVITS request fields but does not load model weights or switch external GPT-SoVITS models. `abilities.mcp.config.save` and `abilities.mcp.discover` are bridged to dedicated MCP server routes; `abilities.mcp.config.open` is client-handled only. `abilities.approvalPolicy.save` is bridged to the profile approval policy route and sends only `defaultMode`. `abilities.workflow.file.import` is bridged to a dedicated workflow file route that saves user-imported JSON without executing the workflow. `voice.records.clear`, `voice.queue.clear`, and `advanced.logs.clear` remain deferred because `main.js` has no corresponding settings command handler.
