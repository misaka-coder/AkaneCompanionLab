...
- **4 client-handled actions** provide real local behavior (expand/collapse UI sections) with `refresh:false` and no backend/Tauri boundary.
...
These fields are verified in scenario 1 (production-shaped snapshot) and scenario 2 (partial degradation) of the runtime probe. The smoke test also confirms all 81 action IDs in `CONTROL_CENTER_ACTIONS` are classified as bridged, client-handled, or deferred — no unclassified action IDs exist, and duplicate page surfaces must stay in the same status category.
