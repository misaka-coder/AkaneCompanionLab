...
Experience list fields are immutable tuples with their declared count/length
limits. These presentation limits do not apply to ordinary business data.
Since V2 P0a, public JSON is preserved without universal 4096-character,
64-item or 64-KiB limits. Operation paths and business fields such as
`token_count` are valid. Invalid JSON and literal private connection values
return specific structured errors; malformed experience structures still fail.
Large model previews link to the same execution's complete JSON through
`inspect_generated_file`. See [the implementation record](plugin_system_v2_progress.md).
...
Plain public `CapabilityResult.content` remains supported for existing plugins.
Its scalar/array/object value is rendered as data, with no experience-complete
claim and no reinterpretation of a business field named `content` as MCP blocks.
