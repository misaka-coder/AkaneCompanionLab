...
状态：设计锁定；F0-F7c、免费公开行情 F7d0-F7d2 与 F9a-F9b 已完成；QQ subscription/watchlist、默认关闭的事件 worker、AI 分析重试、逐项 QQ 投递账本、持久化推送治理、确定性 PNG 图表、MD/PDF/XLSX 金融报告和行情 Provider 解耦已接通；F7d3 AkShare 境内 ETF adapter 为下一切片，真实 Choice 冒烟仍等待账户权限
更新时间：2026-07-11
...
状态：F7d0-F7d2 已完成；Yahoo live 查询在当前网络超时并保留为显式 smoke 项；下一步为 F7d3 AkShare 境内 ETF adapter。
...
F6、F7、F7c、F7d0-F7d2 与 F9a-F9b 已完成。当前优先级是不等待 Choice 审批，继续完成免费公开行情 F7d3-F7d4，让真实指数/ETF 查询、图表和报告可用于演示。上下文恢复后按以下顺序继续：

1. 先读 `docs/public_market_provider_implementation_v1.md` 和 F7d0 离线 fixture；不要重新跑大范围依赖调查，不改基础 `requirements.txt`，不接未文档化快讯接口；
2. 直接执行 F7d3，实现 AkShare 境内 ETF 快照/日线及“手→份额”规范化，推荐提交名 `feat(finance): add public etf market adapter`；
3. 之后再做 `public_market` composite registry/config 接线；
