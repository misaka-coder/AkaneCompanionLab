...
objects. Since V2 P0a, results preserve public JSON values without blanket
string/container/64 KiB limits or field-name/path heuristics. Invalid JSON
returns a specific structured reason. Host exceptions remain private, and
literal private values obtained through connection ports are rejected from
public results. Task operation paths remain usable. Large model previews use
the existing generated-file store and reader; see
[the V2 implementation record](plugin_system_v2_progress.md).
