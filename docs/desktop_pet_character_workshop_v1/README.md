...
- **Phase 8 workshop UI redesign complete**: the workshop is now a quiet production editor with a compact selectable character list, a slim live-status rail, readiness inspector actions, grouped persona fields with a sticky real-save boundary, task-specific portrait/calibration/test surfaces, reduced-motion handling, and explicit desktop/mobile overflow protection. The redesign keeps existing Tauri/backend actions and adds no mock bridge or role-specific theme.
