...
| --- | --- | --- | --- |
| `channelcore-onebot` | Host-neutral OneBot private/group/poke event contracts, text/CQ/attachment/reply/@ normalization, self-id, freshness, atomic replay admission, and group-trigger/follow decisions | Webhook secrets/status mapping, host sessions, product commands, attachment materialization, vision/model/TTS, and outbound delivery | `InboundMessage`, `InboundParseResult`, `ConversationRef`, `ActorRef`, `ReplyRef`, `AttachmentRef`, `EventAdmissionConfig`, `OneBotEventAdmission`, `GroupTriggerPolicy`, `normalize_inbound_event` |
