...
| --- | --- | --- | --- |
| `channelcore-onebot` | Host-neutral OneBot private/group/poke event contracts, text/CQ/attachment/reply/@ normalization, self-id, freshness, and atomic replay admission | Webhook secrets/status mapping, host sessions, product commands, attachment materialization, vision/model/TTS, and outbound delivery | `InboundMessage`, `InboundParseResult`, `ConversationRef`, `ActorRef`, `ReplyRef`, `AttachmentRef`, `EventAdmissionConfig`, `OneBotEventAdmission`, `normalize_inbound_event` |
