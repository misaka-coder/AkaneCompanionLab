...
| `capcore-adapter-mcp` | MCP stdio/Streamable HTTP discovery and calls through the official SDK, optional pooled HTTP sessions, MCP tool to capcore descriptor conversion | Trust policy beyond explicit overrides, provider envelopes, host config UI, secret placeholder hydration | `McpStdioCapabilityAdapter`, `McpStreamableHttpCapabilityAdapter`, transport configs/clients, `McpToolOverride`, `McpToolRecord` |
