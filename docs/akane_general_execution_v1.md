...
- 默认工作区的绝对路径不作为稳定 Prompt 元数据预先注入；但普通主机任务可通过 `pwd`、`find` 等真实命令输出
  发现并使用宿主绝对路径。`cwd` 只接受相对路径/挂载别名是启动目录契约，不代表 Shell 无法访问宿主文件。
...
QQ Shell 还受 `EXECUTION_QQ_ENABLED` 宿主总闸和本轮真实发起人的 `ops`
策略约束。`MASTER_QQ` 对应的主人账号可以发送：
...
- 不执行用户未要求的高风险系统动作。
- 结构化 snapshot、资源 handle、运行日志引用不主动夹带绝对路径、密钥或宿主环境变量名；但在主人授权的
  普通 Shell 任务中，真实命令输出可以包含并继续使用完成任务所需的宿主绝对路径。
...
| `EXECUTION_QQ_ENABLED` | `false` | QQ Shell 宿主总闸；打开后由真实发起人的 `ops` 策略决定执行、询问或关闭 |
...
主人用 `/access ops on|ask|off` 设置发起人权限。详见 `docs/akane_execution_resource_loop_v1.md`。
