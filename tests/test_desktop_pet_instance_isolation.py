...
class DesktopPetInstanceIsolationTests(unittest.TestCase):
    ...
    def test_launcher_decision_rejects_another_instance_and_reuses_matching_default(self) -> None:
        ...
        command = (
            ...
            "$other = Get-AkaneBackendPortDecision -PortInUse $true -Health $health "
            "-ExpectedInstanceId 'instance-a' -ReuseBackend $true -ManagedProcess $true; "
            "$matchingHealth = [pscustomobject]@{ status='ok'; instance_id='instance-a'; root_binding='valid' }; "
            "$reuse = Get-AkaneBackendPortDecision -PortInUse $true -Health $matchingHealth "
            "-ExpectedInstanceId 'instance-a' -ReuseBackend $false -ManagedProcess $true; "
            "$stop = Get-AkaneBackendPortDecision -PortInUse $true -Health $matchingHealth "
            "-ExpectedInstanceId 'instance-a' -ReuseBackend $false -ManagedProcess $true -RestartBackend $true; "
            "@{ other=$other; reuse=$reuse; stop=$stop } | ConvertTo-Json -Compress"
        )
        result = subprocess.run(
        ...
        self.assertEqual(payload, {"other": "reject", "reuse": "reuse", "stop": "stop"})
    ...
    def test_named_data_root_never_copies_local_default_legacy_state(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            local_manifest = local_root / "instances" / "local-default" / "instance.toml"
            self.assertTrue(local_manifest.is_file())
            self.assertIn('character_pack_id = "akane_v1"', local_manifest.read_text(encoding="utf-8"))
            self.assertFalse((named_root / "instances" / "instance-a" / "instance.toml").exists())
