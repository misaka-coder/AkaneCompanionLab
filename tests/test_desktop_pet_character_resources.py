...
class DesktopPetCharacterResourceTests(unittest.TestCase):
    def test_installed_charpack_preserves_long_persona(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            characters_dir = Path(temp_dir)
            pack_dir = characters_dir / "cecilia"
            write_json(pack_dir / "character.json", {"identity": {"id": "cecilia"}})
            persona = "温柔但有边界，亲近不等于服从。\n" * 1000 + "最后原则：保持真实人格。"
            (pack_dir / "persona.md").write_text(persona, encoding="utf-8")
            service = DesktopPetCharacterResourceService(characters_dir=characters_dir)
            for mode in ("qq_text", "desktop_pet"):
                with self.subTest(mode=mode):
                    context = service.build_persona_prompt_context("cecilia", client_mode=mode)
                    self.assertIn(persona, context["reference_context"])
    ...
    def test_character_pack_metadata_drives_qq_persona_context_with_shared_emotion_ids(self) -> None:
        ...
        self.assertIn("默认称呼用户：店长", context["system_context"])
        self.assertNotIn("QQ 端只发送文字", context["system_context"])
        self.assertIn("角色自称：我", context["system_context"])
        self.assertIn("会在 QQ 里陪店长聊天", context["system_context"])
        self.assertIn("当前资源块列出的稳定 id", context["system_context"])
