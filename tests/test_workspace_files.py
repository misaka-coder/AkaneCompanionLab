...
class WorkspaceFileServiceTests(unittest.TestCase):
    ...
    def test_capability_inventory_stops_when_all_presence_facts_are_known(self) -> None:
        for name in ("00-note.md", "01-audio.wav", "02-image.png", "99-unused.bin"):
            (self.root / "Inbox" / name).write_bytes(b"fixture")

        inventory = self.service.capability_inventory()

        self.assertEqual(inventory["files_scanned"], 3)
        self.assertTrue(inventory["truncated"])
        self.assertTrue(all(inventory[key] for key in (
            "has_any_file", "has_document_file", "has_media_file", "has_image_file")))
        # Presence is read again on the next turn, never kept as a stale grant.
        (self.root / "Inbox" / "02-image.png").unlink()
        next_inventory = self.service.capability_inventory()
        self.assertFalse(next_inventory["has_image_file"])
        self.assertFalse(next_inventory["truncated"])

    def test_capability_inventory_continues_for_missing_class_and_honors_limit(self) -> None:
        (self.root / "Inbox" / "00-note.md").write_bytes(b"fixture")
        (self.root / "Inbox" / "01-image.png").write_bytes(b"fixture")
        (self.root / "Outputs" / "last.wav").write_bytes(b"fixture")

        limited = self.service.capability_inventory(max_files=1)
        self.assertEqual(limited["files_scanned"], 1)
        self.assertTrue(limited["truncated"])
        self.assertFalse(limited["has_media_file"])
        complete = self.service.capability_inventory()
        self.assertTrue(complete["has_media_file"])
        self.assertEqual(complete["files_scanned"], 3)

    def test_capability_inventory_does_not_count_linked_material(self) -> None:
        outside = self.base_dir / "outside.png"
        outside.write_bytes(b"fixture")
        linked = self.root / "Inbox" / "linked.png"
        try:
            linked.symlink_to(outside)
        except (OSError, NotImplementedError):
            self.skipTest("symlink creation unavailable")
        (self.root / "Inbox" / "note.md").write_bytes(b"fixture")
        (self.root / "Inbox" / "song.wav").write_bytes(b"fixture")

        inventory = self.service.capability_inventory()

        self.assertFalse(inventory["has_image_file"])
