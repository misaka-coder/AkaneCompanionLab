...
from companion_v01.llm_runtime import ChatJSONResult, LLMRuntime, ModelBundle, NDJSONCallResult
...
class LLMClientConfigTests(unittest.TestCase):
    ...
    def test_chat_provider_failover_prefers_dedicated_aux_fallback_over_aux_primary(self) -> None:
        runtime = LLMRuntime.__new__(LLMRuntime)
        runtime.instance_id = "finance"
        runtime.settings = BotSettingsView(
        ...
        recovered = ChatJSONResult(parsed={"speech": "recovered"}, raw_text='{"speech":"recovered"}')
    ...
    def test_aux_json_failover_retries_once_through_dedicated_provider(self) -> None:
        ...
        runtime.settings = BotSettingsView(llm_aux_failover_enabled=True)
        runtime._bundle_lock = threading.RLock()
        runtime._metrics_lock = threading.RLock()
        runtime._metrics = {}
        runtime.aux = ModelBundle(
        ...
        recovered = ChatJSONResult(parsed={"summary": "recovered"}, raw_text='{"summary":"recovered"}')
        ...
        self.assertIs(call.call_args_list[0].kwargs["bundle"], runtime.aux)
        self.assertIs(call.call_args_list[1].kwargs["bundle"], runtime.aux_failover)
        self.assertEqual(runtime.snapshot_metrics()["aux_provider_failovers"], 1)
        self.assertEqual(runtime.snapshot_metrics()["aux_provider_failover_successes"], 1)

    def test_aux_json_success_does_not_touch_failover_provider(self) -> None:
        runtime = LLMRuntime.__new__(LLMRuntime)
        runtime.settings = BotSettingsView(llm_aux_failover_enabled=True)
        runtime._bundle_lock = threading.RLock()
        runtime._metrics_lock = threading.RLock()
        ...
        primary = ChatJSONResult(parsed={"summary": "ok"}, raw_text='{"summary":"ok"}')
        ...
        self.assertEqual(call.call_count, 1)
        self.assertNotIn("aux_provider_failovers", runtime.snapshot_metrics())

    def test_aux_ndjson_empty_primary_retries_without_duplicate_events(self) -> None:
        runtime = LLMRuntime.__new__(LLMRuntime)
        runtime.instance_id = "personal"
        runtime.settings = BotSettingsView(llm_aux_failover_enabled=True)
        runtime._bundle_lock = threading.RLock()
        ...
        empty = NDJSONCallResult([], [], 1.0, False, True, "", "")
        recovered = NDJSONCallResult(
            [{"type": "verified"}],
            [{"index": 0, "type": "verified", "elapsed_ms": 2.0}],
            2.0,
            False,
            True,
            "",
        ...
        with patch.object(runtime, "_call_ndjson", side_effect=[empty, recovered]) as call:
            result = runtime.call_aux_ndjson(
                system_prompt="system",
                user_prompt="verify",
            )

        self.assertEqual(result.events, [{"type": "verified"}])
