...
from companion_v01.tts_connection_source import resolve_tts_connection
from companion_v01.store import MemoryStore
from companion_v01.vision_service import VisionObservationService


class BotSettingsViewTests(unittest.TestCase):
    def test_voice_snapshot_isolated_in_private_tts_connection(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            save_provider_config(base_dir=directory, profile_user_id="owner", provider_id="provider.tts.gpt_sovits.local",
                payload={"enabled": True, "endpoint": "http://127.0.0.1:18001"})
            save_voice_profile_config(base_dir=directory, profile_user_id="owner", voice_profile_id="voice",
                payload={"enabled": True, "refAudioPath": str(Path(directory) / "reference.wav"), "promptText": "test"})
            def options(settings):
                invocation = SimpleNamespace(arguments={"voice": {"provider": "provider.tts.gpt_sovits.local", "profile_id": "voice"}},
                    context=SimpleNamespace(profile_user_id="owner", client_mode="desktop_pet"), host_parameters={}, issued_resources={})
                result = resolve_tts_connection(SimpleNamespace(capability_config_base_dir=directory, settings=settings), None, invocation)
                self.assertTrue(result.ok, result)
                return result.options["client"]
            a, b = options(settings_a), options(settings_b)
        fields = ("timeout_seconds", "text_lang", "media_type", "streaming_mode", "speed_factor")
        self.assertEqual(tuple(a[key] for key in fields), (11.0, "zh", "wav", False, 0.8))
        self.assertEqual(tuple(b[key] for key in fields), (23.0, "en", "mp3", True, 1.2))
        self.assertNotEqual(a["text_split_method"], b["text_split_method"])
