...
class _PromptContextMemcoreManager:
    def resolve_tool_exposure_snapshot(self, *, snapshot, compaction_generation, **_kwargs):
        # This fixture isolates prompt ordering/budget tests from persistence.
        # Real generation storage and restart guarantees are covered by
        # test_tool_exposure_memcore and recorded provider-wire tests.
        return {"ok": True, "status": "created", "snapshot": snapshot,
...
class MemcoreIntegrationTests(unittest.TestCase):
    def test_first_proactive_turn_refreshes_new_capability_catalog_without_user_source(self) -> None:
        class CatalogEngine(_PromptContextEngine):
            def _build_tool_prompt_context_sections(self, **_kwargs):
                return {"execution_context": "", "catalog_context": "【可按需加载的 Skills】\n- screen-test", "catalog_status": "ready", "round_context": ""}
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                scope = {"profile_user_id": "u1", "session_id": "new-screen-session", "character_pack_id": "char"}
                opened = manager.begin_hidden_host_turn(**scope)
                self.assertTrue(opened["ok"], opened)
                engine = CatalogEngine(memcore_manager=manager)
                with patch.object(config, "MEMORY_BACKEND", "memcore"):
                    context = response_builder.prepare_context(engine, user_message="screen observation",
                ...
                self.assertIn("screen-test", engine.prompt_builder.calls[0]["capability_catalog_context"])
                self.assertTrue(context["prompt_context_lifecycle"]["capability_catalog"]["changed"])
                self.assertIn("screen observation", str(context))
            finally:
                manager.close()
    ...
    def test_manager_preserves_exact_timeline_coverage_and_expands_raw_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manager.append_standalone_message(
                {
                    "source_id": "exact-1148",
                    "content": "随后又约定一起去吃早茶",
                    "timestamp": _ts(2026, 8, 3, 11, 48),
                    "memory_metadata": {},
                },
                role="assistant",
                profile_user_id="u1",
            ...
            timeline = manager.read_memory_timeline(
                profile_user_id="u1",
                session_id="group:42",
                character_pack_id="char",
            ...
            second_page = manager.read_memory_timeline(
                profile_user_id="u1",
                session_id="group:42",
                character_pack_id="char",
                cursor=str(first_page["coverage"]["next_cursor"]),
            )

            self.assertTrue(timeline["ok"], timeline)
            self.assertEqual(timeline["projection"], "conversation")
            self.assertTrue(timeline["coverage"]["complete"])
            ...
            self.assertNotIn("content", expanded["result"])
            self.assertEqual(expanded["result_projection"], "rendered_text_with_navigation_metadata")
            ...
            try:
                opened = manager.begin_input_turn(
                    ...
                    external_event={
                        "event_type": "finance",
                        "source": "公开快讯",
                        "data": {"title": "消费政策方向更新", "summary": "尚无执行细节。"},
                    },
                    profile_user_id="u1",
    ...
    def test_retrieve_memory_tool_empty_result_preserves_visible_evidence_guidance(self) -> None:
        memcore_manager = _ToolFakeMemcoreManager(
            {
                "operation": "retrieve_memory",
                "ok": True,
        ...
        self.assertIn("没有返回额外匹配", result.followup_context)
        self.assertIn("不等于当前可见上下文中没有相关记录", result.followup_context)
        self.assertIn("若已有足够的明确证据，直接据此回答", result.followup_context)
        self.assertIn("只有现有证据仍不足", result.followup_context)
        self.assertIn("若核对后仍无明确证据", result.followup_context)
        self.assertNotIn("请自然说明没有想起可靠线索", result.followup_context)
        self.assertIn("status=empty；returned=0", result.followup_context)
        ...
        self.assertNotIn("verifier_output", state)
        self.assertNotIn("verifier_timing", state)
        self.assertEqual(state["retrieval_diagnostics"]["retrieval_status"], "empty")
        self.assertIsNotNone(result.followup_envelope)
        ...
        self.assertEqual(shaped, result.followup_context)
    ...
    def test_capability_catalog_update_remains_in_persistent_turn(self) -> None:
        ...
        self.assertEqual(len(result["memcore_projection_read"]["current_turn_messages"]), 2)
        self.assertIn("skill-c", result["post_user_turns"][0]["content"])
        self.assertNotIn("【能力目录更新】", str(result.get("ephemeral_turns", [])))

    def test_capability_catalog_projection_keeps_all_updates_in_order(self) -> None:
        projection = {
            "history_turns": [
        ...
        self.assertIn("skill-b", rendered)
        self.assertLess(rendered.index("skill-b"), rendered.index("skill-c"))
    ...
    def test_read_memory_timeline_tool_uses_memcore_adapter_in_memcore_mode(self) -> None:
        ...
        self.assertEqual(
            result.state_updates["memory_timeline"]["coverage"]["next_cursor"],
            "timeline-v1:next",
        )
        self.assertIsNotNone(result.followup_envelope)
        self.assertTrue(result.followup_envelope.producer_bounded)
        self.assertEqual(
            result.followup_envelope.continuation,
            {"cursor": "timeline-v1:next"},
        )

    def test_read_memory_timeline_forwards_exact_range_and_cursor_without_reinterpreting_them(self) -> None:
        memcore_manager = _TimelineMemcoreManager()
        service = MemcoreTimelineToolService(legacy_service=None, memcore_manager=memcore_manager)
        handler = ReadMemoryTimelineToolHandler(timeline_service=service)
        exact = handler.normalize_call(
            {
                "type": "read_memory_timeline",
                "time_range": {"start_at": "2026-08-03 11:00", "end_at": "2026-08-03 12:00"},
                "projection": "conversation",
                "page_token_budget": 12000,
            }
        )
        continuation = handler.normalize_call(
            {"type": "read_memory_timeline", "cursor": "timeline-v1:next"}
        )

        self.assertIsNotNone(exact)
        self.assertIsNotNone(continuation)
        assert exact is not None and continuation is not None
        with patch.object(config, "MEMORY_BACKEND", "memcore"):
            handler.execute(call=exact, context=ToolExecutionContext("u1", "group:42", 0, {}))
            handler.execute(call=continuation, context=ToolExecutionContext("u1", "group:42", 0, {}))

        self.assertEqual(
            memcore_manager.calls[0]["time_range"],
            {"start_at": "2026-08-03 11:00", "end_at": "2026-08-03 12:00"},
        )
    ...
    def test_empty_projected_page_still_exposes_lossless_continuation(self) -> None:
        ...
        rendered = service.render_tool_context(
            {
                "backend": "memcore",
                "status": "empty",
                "selector_mode": "time_range",
                "time_range": {
                    "start_at": "2026-08-03T11:00:00+08:00",
                    "end_at": "2026-08-03T12:00:00+08:00",
                },
                "text": "",
                "coverage": {
                    "complete": False,
                    "next_cursor": "timeline-v1:next-page",
                },
            }
        )
