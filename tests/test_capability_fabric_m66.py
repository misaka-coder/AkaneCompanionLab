...
class CapabilityFabricM66Tests(unittest.TestCase):
    ...
    def test_broker_requires_exact_instance_live_lease_and_real_result(self) -> None:
        ...
        with TestClient(self._app(service)) as client:
            with client.websocket_connect(
                ...
                broker.resource_policy = ExecutionResourcePolicy.from_config({"max_input_bytes": 8})
                blocked = broker.execute(spec=OPEN_BROWSER_TOOL_SPEC, receipt_value=receipt.as_dict(),
                    invocation_id="call_resource_blocked", arguments={"url": "https://example.com"}, ledger_scope="owner\x1fsession")
                self.assertEqual(blocked.status, "resource_exhausted")
                self.assertEqual(blocked.data["resource_limit"]["execution_status"], "not_started")
                broker.resource_policy = ExecutionResourcePolicy()
                replay = broker.execute(spec=OPEN_BROWSER_TOOL_SPEC, receipt_value=receipt.as_dict(),
                    invocation_id="call_resource_blocked", arguments={"url": "https://example.com"}, ledger_scope="owner\x1fsession")
                self.assertIs(replay, blocked)
                ...
                rendered, envelope = _execute_open_browser_with_broker(SimpleNamespace(executor_broker=broker),
                    invocation=ToolInvocation(OPEN_BROWSER_TOOL_SPEC.capability_id, {"url": "https://example.com"},
                        id="call_resource_blocked", execution_receipt=receipt.as_dict()),
                    profile_user_id="owner", session_id="session")
                self.assertEqual(rendered.capability_result.status, "resource_exhausted")
    ...
    def test_disconnect_after_wire_dispatch_is_unknown_with_or_without_ack(self) -> None:
        # Both cases already receive the invoke frame on the client. Missing
        # acceptance cannot prove the client did not execute it before disconnect.
        for acknowledged in (False, True):
            with self.subTest(acknowledged=acknowledged):
                ...
                with TestClient(self._app(service)) as client:
                    ...
                    worker.join(timeout=5)
                self.assertFalse(worker.is_alive())
                self.assertEqual(result_box["result"].status, "execution_unknown")
    ...
    def test_cloud_launcher_has_no_backend_process_branch(self) -> None:
        ...
        self.assertLess(
            source.index("Stop-AkaneDesktopProcesses -ExePath $instanceExe", build_guard),
    ...
    def test_personal_cloud_launcher_owns_tunnel_and_offer_verification(self) -> None:
        ...
        voice_source = (ROOT / "scripts" / "akane_voice_runtime.ps1").read_text(encoding="utf-8")
        self.assertIn("scripts\\akane_voice_runtime.ps1", source)
        self.assertIn("Ensure-AkaneGptSoVitsApi", source)
        ...
        self.assertIn("AKANE_GPT_SOVITS_ROOT", voice_source)
        self.assertIn("api_v2.py", voice_source)
        self.assertIn("GPT_SoVITS/configs/tts_infer.yaml", voice_source)
        self.assertIn("gpt_sovits_api.pid", source)
        self.assertIn("gpt_sovits_reverse_tunnel.pid", source)
        self.assertIn("Get-AkaneGptSoVitsApi", voice_source)
        ...
        self.assertIn("Get-CimInstance Win32_Process", voice_source)
        self.assertIn("$actualExecutableValue = ([string]$process.ExecutablePath).Trim()", voice_source)
        self.assertIn("if (-not $actualExecutableValue -or -not $expectedExecutableValue)", voice_source)
        self.assertIn("catch {", source)
        self.assertIn("Stop-Process -Id $storedProcessId -Force", voice_source)
