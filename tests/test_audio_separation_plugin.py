    ...
    async def exercise_jobs(self, *, root, engine, files, attachments, source_id, service):
        ...
        def submit(invocation, source, fmt="mp3"):
            return runtime.submit(
                ...
                call={"type": CAPABILITY_ID, "arguments": {"source_id": source, "output_format": fmt}},
                context=context,
        ...
        try:
            ...
            success_id = submit("separation-job", source_id)
            self.assertLess(time.monotonic() - start, 1)
            await settle()
            success = jobs.get(success_id, owner=owner)
            self.assertEqual(success.status, "succeeded", success)
            self.assertEqual(len(success.artifacts), 2)
            self.assertEqual(submit("separation-job", source_id), success_id)
            ...
            cancel_id = submit("cancel-separation", item["attachment_id"], "wav")
            deadline = time.monotonic() + 30
            while not list((root / "copies").rglob("prepared-*/input.wav")) and time.monotonic() < deadline:
                await asyncio.sleep(0.05)
            self.assertTrue(list((root / "copies").rglob("prepared-*/input.wav")), "No real model input prepared")
            ...
            # the converter, independently exercised for both real output files.
    ...
    async def test_real_market_install_model_conversion_scope_and_lifecycle(self):
        with (
            ...
            try:
                ...
                timeline_vocals = await asyncio.to_thread(
                ...
                self.assertEqual(timeline_vocals["status"], "ready", timeline_vocals)
                with wave.open(timeline_vocals["absolute_path"], "rb") as output:
                    self.assertEqual(output.getnframes(), 88200)
                ...
                self.assertNotIn(CAPABILITY_ID, engine._resolve_tool_handlers())
                self.discovery_snapshot(engine, installed=False)
                timeline_disabled = prepare_timeline_vocals(
                    engine, profile_user_id="owner", session_id="session", source_id=item["attachment_id"]
                )
                self.assertEqual(timeline_disabled["reason"], "separation_plugin_unavailable")
