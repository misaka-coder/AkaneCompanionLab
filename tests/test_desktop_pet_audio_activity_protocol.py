...
class TestLayer3NormalizeFinalOutput:
    ...
    def test_missing_activity_does_not_infer_play_from_user_text(self, engine_desktop):
        """缺少显式 activity 时，用户文字不能自动变成设备动作。"""
        result = normalize_final_output(
            engine_desktop,
    ...
    def test_missing_activity_stays_missing_for_non_media_text(self, engine_desktop):
        """普通文字也不应产生 activity。"""
        result = normalize_final_output(
            engine_desktop,
            result=_LLM_RESULT_NO_ACTIVITY,
    ...
    def test_negated_music_request_stays_without_activity(self, engine_desktop):
        """否定句保持无动作。"""
        result = normalize_final_output(
            engine_desktop,
            result=_LLM_RESULT_NO_ACTIVITY,
    ...
    def test_music_keyword_does_not_infer_pause(self, engine_desktop):
        """“停一下”不能绕过显式动作协议。"""
        result = normalize_final_output(
            engine_desktop,
            result=_LLM_RESULT_NO_ACTIVITY,
