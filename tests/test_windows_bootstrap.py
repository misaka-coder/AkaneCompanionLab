...
class WindowsBootstrapContractTests(unittest.TestCase):
    ...
    def test_bootstrap_prepares_core_and_keeps_auto_mode_on_desktop(self) -> None:
        ...
        self.assertIn("silently switch", source)
        self.assertIn("ForcePythonInstall", source)
        self.assertIn("only an incomplete environment or the", source)
        self.assertIn("Start-WebMode", source)
        self.assertIn("Start-DesktopMode", source)
        self.assertIn('[ValidateSet("Start", "Status", "Stop", "Restart", "Doctor")]', source)
        self.assertIn("lifecycle_action_requires_local_backend", source)
        self.assertIn("SkipDesktop = $true", source)
    ...
    def test_desktop_release_launchers_have_explicit_build_control(self) -> None:
        root_launcher = (ROOT / "start_akane_next.ps1").read_text(encoding="utf-8")
        direct_launcher = (ROOT / "desktop_pet_next" / "scripts" / "start-next.ps1").read_text(encoding="utf-8")
        ...
        self.assertIn("$shouldBuild = (-not $releaseExists) -or $Rebuild", root_launcher)
        self.assertIn("use -Rebuild explicitly", root_launcher)
