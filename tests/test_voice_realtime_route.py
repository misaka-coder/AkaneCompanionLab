...
class _CoordinatorFactory:
    ...
    def __call__(self, request: Any) -> VoiceRealtimeCoordinatorResolution:
        ...
        if request.output_mode == VOICE_PLAYBACK_OUTPUT_MODE:
            ...
            delivery_channel.input_bridge = bridge
...
class VoiceRealtimeRouteTests(unittest.TestCase):
    def test_legacy_interaction_closes_as_input_without_fake_response(self) -> None:
        ...
        with TestClient(self._app(factory=factory)) as client:
            with client.websocket_connect("/voice/realtime") as websocket:
                websocket.send_json(_open_payload(disposition="interaction", output={"mode": VOICE_PLAYBACK_OUTPUT_MODE}))
                self.assertEqual(websocket.receive_json()["type"], "server.ready")
                websocket.send_json({"type": "client.audio", "sequence": 0, "audio_clock_ms": 0})
                websocket.send_bytes(b"\x01\x00" * 160)
                self.assertEqual(websocket.receive_json()["type"], "server.partial")
                ...
                self.assertEqual(websocket.receive()["code"], 1000)
        self.assertEqual(factory.delivery_channels[0].close_reason, "voice_interaction_committed")
