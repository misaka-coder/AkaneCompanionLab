...
class RetrieveMemoryToolHandlerTests(unittest.TestCase):
    def test_prompt_instruction_frames_retrieval_as_deep_memory_space(self) -> None:
        ...
        self.assertIn('{"type":"retrieve_memory"}', instruction)
        schema = json.loads(instruction.split("业务参数完整 JSON Schema：", 1)[1])
...
class AdapterCapabilityToolHandlerTests(unittest.TestCase):
    ...
    def test_adapter_capability_prompt_schema_uses_capcore_projection(self) -> None:
        ...
        schema = json.loads(instruction.split("业务参数完整 JSON Schema：", 1)[1])
        self.assertEqual(schema["properties"]["text"]["description"], "Text to echo")
...
class WebSearchToolHandlerTests(unittest.TestCase):
    ...
    def test_prompt_instruction_searches_current_public_facts_without_explicit_search_word(self) -> None:
        ...
        self.assertIn('{"type":"web_search"}', instruction)
        schema = json.loads(instruction.split("业务参数完整 JSON Schema：", 1)[1])
        self.assertIn("batch_search", schema["properties"]["action"]["enum"])
        self.assertEqual(schema["properties"]["action"]["enum"], ["search", "batch_search", "extract", "get_sub_domains"])
...
class BrowserPageToolHandlerTests(unittest.TestCase):
    ...
    def test_tool_spec_action_enum_is_the_single_model_visible_contract(self) -> None:
        ...
        from companion_v01.personal_browser_contract import ACTIONS
        self.assertEqual(set(enum_actions), set(handler.ALLOWED_ACTIONS) | set(ACTIONS))
        minimal_calls = {
            ...
            "download_status": {"type": "browser_page", "action": "download_status"},
            "capabilities": {"type":"browser_page", "action":"capabilities"},
            "hover": {"type":"browser_page", "action":"hover", "selector":"#target"},
            "set_checked": {"type":"browser_page", "action":"set_checked", "selector":"#target", "checked":True},
            "select_option": {"type":"browser_page", "action":"select_option", "selector":"#target", "values":["a"]},
            "upload": {"type":"browser_page", "action":"upload", "selector":"#target", "files":["file_001"]},
            "run_actions": {"type":"browser_page", "action":"run_actions", "actions":[{"action":"click", "selector":"#target"}]},
        }
        for action in handler.ALLOWED_ACTIONS:
    ...
    def test_normalize_call_accepts_public_url_and_bounds_text_size(self) -> None:
        ...
        read_current = handler.normalize_call({"type": "browser_page", "action": "read_text"})
        ...
        elements = handler.normalize_call({"type": "browser_page", "action": "elements", "element_limit": 99})
        ...
        snapshot = handler.normalize_call({"type": "browser_page", "action": "snapshot"})
        self.assertIsNotNone(snapshot)
        assert snapshot is not None
        self.assertEqual(snapshot["action"], "snapshot")

        # Historical action and parameter aliases remain normalized for stored
        # callers; the public schema still exposes only canonical names.
        legacy_open = handler.normalize_call({"type": "browser_page", "action": "open", "link": "https://example.com"})
        self.assertIsNotNone(legacy_open)
        assert legacy_open is not None
        self.assertEqual(legacy_open["action"], "navigate")
        legacy_fill = handler.normalize_call(
            {"type": "browser_page", "action": "type", "target": "input[name='q']", "value": "Akane"}
        )
        self.assertIsNotNone(legacy_fill)
        assert legacy_fill is not None
        self.assertEqual(legacy_fill["action"], "fill")
        self.assertEqual(legacy_fill["selector"], "input[name='q']")
        self.assertEqual(legacy_fill["text"], "Akane")
        legacy_scroll = handler.normalize_call({"type": "browser_page", "action": "scroll", "delta": -100})
        self.assertIsNotNone(legacy_scroll)
        assert legacy_scroll is not None
        self.assertEqual(legacy_scroll["scroll_delta"], -100)
        ...
        self.assertIsNotNone(handler.normalize_call({"type": "browser_page", "action": "click", "selector": "#password"}))
