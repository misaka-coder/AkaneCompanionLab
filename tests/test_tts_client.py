...
class GptSovitsTTSClientTests(unittest.TestCase):
    ...
    def test_character_runtime_resolves_each_segment_through_public_service(self) -> None:
        from types import SimpleNamespace
        from companion_v01.tts_service import SynthesizedTTSResult
        preference = {"provider": "gpt_sovits", "profileId": "first"}
        engine = SimpleNamespace(desktop_pet_character_resources=SimpleNamespace(
            build_character_voice_preference=lambda _: dict(preference)))
        calls = []
        async def public_service(**request):
            calls.append(request)
            return SynthesizedTTSResult(b"fixture", "audio/wav", request["voice"]["provider"],
                request["voice"]["profile_id"], "", "", "")
        resolved = resolve_character_tts_client(engine=engine, profile_user_id="master", session_id="desktop", character_pack_id="reimu")
        with patch("companion_v01.tts_provider_runtime.synthesize_tts_service", side_effect=public_service):
            self.assertEqual(asyncio.run(resolved.synthesize("第一句")).voice_profile_id, "first")
            preference["profileId"] = "second"
            self.assertEqual(asyncio.run(resolved.synthesize("第二句")).voice_profile_id, "second")
        self.assertEqual([call["voice"]["profile_id"] for call in calls], ["first", "second"])
        self.assertTrue(all(call["context"].character_pack_id == "reimu" for call in calls))
