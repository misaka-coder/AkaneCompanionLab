...
class VoiceRuntimeTTSExecutorTests(unittest.TestCase):
    ...
    def test_untyped_service_failure_remains_unknown_across_recovery(self) -> None:
        class ServiceClient(_TTSClient):
            async def synthesize_command(self, text, **_context):
                self.calls.append(text)
                raise RuntimeError("private transport failure after provider dispatch")

        with tempfile.TemporaryDirectory() as temp_dir:
            text_artifacts, audio_artifacts = self._ports(Path(temp_dir))
            written = text_artifacts.put_text(artifact_key="response-1:0", text="不能确认服务终态。", media_type="text/plain")
            command = _command()
            command["payload"]["text_artifact_ref"] = written.artifact_ref
            client = ServiceClient()
            executor = AkaneVoiceTTSCommandExecutor(tts_client=client,
                text_artifacts=text_artifacts, audio_artifacts=audio_artifacts,
                conversation_id="conversation-1", conversation_generation=1)
            for result in (executor.execute(command, _snapshot()), executor.recover(command, _snapshot())):
                self.assertFalse(result.outcome_known)
                self.assertEqual(result.reason, "voice_tts_outcome_unconfirmed")
                self.assertEqual(result.observations, ())
                self.assertNotIn("private transport", str(result))
            self.assertEqual(client.calls, ["不能确认服务终态。"])
