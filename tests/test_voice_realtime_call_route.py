...
class _CallDeliveryChannel:
    def __init__(self, *, voice_turn_id: str) -> None:
        ...
        self._taken = False
        self.input_bridge: VoiceASRSessionBridge | None = None

    def interaction_committed(self) -> bool:
        return self.input_bridge is not None and self.input_bridge.committed_disposition == "interaction"
...
class _CallRuntime:
    ...
    def create_turn(self, request: Any) -> VoiceRealtimeCoordinatorResolution:
        ...
        if request.output_mode == VOICE_PLAYBACK_OUTPUT_MODE:
            ...
            delivery_channel.input_bridge = coordinator.bridge
...
class VoiceRealtimeCallRouteTests(unittest.TestCase):
    def test_interaction_has_explicit_terminal_and_call_can_listen_again(self) -> None:
        ...
        with TestClient(self._app(factory=factory)) as client:
            with client.websocket_connect("/voice/realtime") as websocket:
                payload = _call_open_payload(output_mode=VOICE_PLAYBACK_OUTPUT_MODE)
                payload["disposition"] = "interaction"
                websocket.send_json(payload)
                self.assertEqual(websocket.receive_json()["type"], "server.call.ready")
                for turn_number in (1, 2):
                    self._complete_text_turn(websocket, turn_number)
                    interaction = websocket.receive_json()
                    self.assertEqual(interaction["type"], "server.turn.interaction")
                    self.assertEqual(interaction["state"], "committed")
                    self.assertEqual(interaction["disposition"], "interaction")
                    self.assertEqual(interaction["voice_turn_id"], f"voice-turn-call-route-{turn_number}")
                websocket.send_json({"type": "client.call.close"})
                self.assertEqual(websocket.receive_json()["type"], "server.call.closed")
        self.assertEqual(factory.open_count, 1)
        self.assertTrue(
            all(
                channel.close_reason == "voice_interaction_committed"
                for channel in factory.calls[0].delivery_channels.values()
            )
        )
