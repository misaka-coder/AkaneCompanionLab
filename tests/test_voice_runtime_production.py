...
from companion_v01.voice_runtime.realtime_transport import (
...
class _SlowTTSClient(_TTSClient):
    def __init__(self) -> None:
        super().__init__()
...
class _AheadOfTTSThinkingEngine(_ThinkingEngine):
    segments = ("第一句先播放。", "第二句随后播放。", "第三句也已经生成！", "最后一句不会遗漏。")

    def __init__(self, manager: MemcoreManager) -> None:
        super().__init__(manager)
        self.stream_consumed = threading.Event()

    def process_voice_turn_stream(self, **kwargs: Any):
        self.calls.append(dict(kwargs))
        for index, text in enumerate(self.segments):
            yield {"type": "speech_segment", "index": index, "text": text}
        self.stream_consumed.set()
        yield {"type": "final", "payload": {"speech": "".join(self.segments)}}
...
class VoiceRuntimeProductionTests(unittest.TestCase):
    def test_presentation_reaches_both_transports_before_audio_and_only_once(self) -> None:
        for version in (1, 2):
            with self.subTest(version=version), tempfile.TemporaryDirectory() as temp_dir:
                ...
                class Engine(_ThinkingEngine):
                    def process_voice_turn_stream(self, **kwargs: Any):
                        yield {"type": "speech_segment", "index": 0, "text": "好的。"}
                        yield {
                            "type": "final",
                            "payload": {
                                "speech": "好的。",
                                ...
                                "activity": {
                                    "action": "pause",
                                    "source_id": "system_media_current",
                                    "cachedPath": "private",
                                },
                                "memory_metadata": {},
                                "trace_id": "private",
                ...
                service = self._service(
                    root=root, manager=manager, adapter=_Adapter(), engine=Engine(manager), tts_client=tts
                )
                try:
                    request = _open_request(output_mode=VOICE_PLAYBACK_OUTPUT_MODE)
                    resolved = service.create_coordinator(request)
                    asyncio.run(_commit_realtime_turn(resolved.coordinator))
                    self.assertTrue(tts.started.wait(timeout=2.0))
                    channel = resolved.delivery_channel
                    deadline = time.monotonic() + 2.0
                    while not any(r.generation_terminal for r in channel.host.snapshot.responses.values()):
                        self.assertLess(time.monotonic(), deadline)
                        time.sleep(0.005)
                    self.assertIsNone(channel.take_outbound(), "Synthesis remains blocked")
                    messages = []
                    ...
                    async def exercise() -> None:
                        await asyncio.wait_for(channel.wait_activity(), timeout=0.5)
                        kwargs = dict(
                        ...
                        if version == 1:
                            transport = VoiceRealtimeWebSocketSession(coordinator_factory=None, **kwargs)
                            transport.open_request = request
                            transport.delivery_channel = channel
                            await transport._handle_delivery_activity()
                            await transport._handle_delivery_activity()
                        else:
                            transport = VoiceRealtimeCallWebSocketSession(call_factory=None, **kwargs)
                            transport.delivery_channels[request.voice_turn_id] = channel
                            await transport._handle_delivery_activity(request.voice_turn_id)
                            await transport._handle_delivery_activity(request.voice_turn_id)

                    asyncio.run(exercise())
                    self.assertEqual(len(messages), 1)
                    self.assertEqual(messages[0]["type"], "server.response.presentation")
                    self.assertEqual(messages[0]["voice_turn_id"], request.voice_turn_id)
                    self.assertEqual(
                        messages[0]["presentation"],
                        {
                            "emotion": "happy",
                            "activity": {"action": "pause", "source_id": "system_media_current"},
                        },
                    )
                    self.assertEqual(tts.calls, ["好的。"])
                finally:
                    tts.release.set()
                    service.close()
                    manager.close()

    def test_cancelled_or_invalid_final_never_presents(self) -> None:
                    ...
                    def process_voice_turn_stream(self, **kwargs: Any):
                        ...
                        yield {
                            ...
                            "payload": {
                                "speech": "好的。",
                                "emotion": "happy",
                                "memory_metadata": [] if invalid else {},
                            },
                        }

                service = self._service(
                    root=root, manager=manager, adapter=_Adapter(), engine=Engine(manager), tts_client=_TTSClient()
                )
                try:
                    ...
                    if not invalid:
                        self.assertTrue(resolved.coordinator.cancel_response(reason="new_user_turn").ok)
                    self.assertIsNone(resolved.delivery_channel.take_presentation())
                finally:
                    service.close()
                    manager.close()

    def test_text_only_completion_flushes_presentation_before_terminal(self) -> None:
        for version in (1, 2):
            with self.subTest(version=version), tempfile.TemporaryDirectory() as temp_dir:
                root = Path(temp_dir)
                manager = self._manager(root)
                ...
                class Engine(_ThinkingEngine):
                    def process_voice_turn_stream(self, **kwargs: Any):
                        yield {"type": "final", "payload": {"speech": "好的。", "emotion": "happy"}}
                ...
                try:
                    ...
                    self.assertEqual(outcome.delivery_status, "text_only")
                    messages = []
                    ...
                    async def exercise() -> None:
                        ...
                        if version == 1:
                            ...
                            transport.open_request = request
                            transport.delivery_channel = channel
                            await transport._finish_delivered_response(outcome)
                        else:
                            transport = VoiceRealtimeCallWebSocketSession(call_factory=None, **kwargs)
                            transport.delivery_channels[request.voice_turn_id] = channel
                            await transport._finish_delivered_response(request.voice_turn_id, outcome)

                    asyncio.run(exercise())
                    self.assertEqual(
                        [m["type"] for m in messages], ["server.response.presentation", "server.response.completed"]
                    )
                    self.assertEqual(messages[0]["presentation"], {"emotion": "happy"})
                finally:
                    service.close()
                    manager.close()

    def test_late_semantic_commit_preserves_backchannel_or_starts_takeover(self) -> None:
        for decision in ("backchannel", "takeover", "finish_unit", "failed"):
            takeover = decision in {"takeover", "finish_unit"}
            stop_now = decision == "takeover"
            with self.subTest(decision=decision), tempfile.TemporaryDirectory() as temp_dir:

                class DelayedSemanticLLM(_SemanticLLM):
                    ...
                    def call_chat_json_result(self, **kwargs: Any) -> Any:
                        self.started.set()
                        if not self.release.wait(timeout=5.0):
                            raise RuntimeError("test semantic release timeout")
                        result = super().call_chat_json_result(**kwargs)
                        if takeover:
                            result.parsed.update(
                                playback_action="stop_now" if stop_now else "finish_current_unit_then_stop",
                                input_action="take_over",
                            )
                        if decision == "failed":
                            result.error = "test provider unavailable"
                        return result

                root = Path(temp_dir)
                manager = self._manager(root)
                llm = DelayedSemanticLLM()
                engine = _SemanticThinkingEngine(manager, llm=llm)
                service = self._service(
                    root=root, manager=manager, adapter=_Adapter(), engine=engine, tts_client=_TTSClient()
                )
                try:
                    source = service.create_coordinator(_open_request(output_mode=VOICE_PLAYBACK_OUTPUT_MODE))
                    asyncio.run(_commit_realtime_turn(source.coordinator))
                    self.assertTrue(service.wait_idle(timeout=5.0))
                    channel = source.delivery_channel
                    ...
                    self.assertTrue(
                        channel.acknowledge(
                            "client.playback.started",
                            {
                                ...
                                "resume_token": "overlap-resume",
                            },
                        ).ok
                    )
                    request = _open_request(voice_turn_id="late-semantic-turn", output_mode=VOICE_PLAYBACK_OUTPUT_MODE)
                    overlap = service.create_coordinator(request)

                    async def finalize_overlap() -> Any:
                        self.assertTrue((await overlap.coordinator.open()).ok)
                        self.assertTrue(overlap.coordinator.suspect_interruption(audio_clock_ms=300).ok)
                        fed = await overlap.coordinator.feed_pcm_frame(b"\x01\x00" * 160, sequence=0, audio_clock_ms=0)
                        self.assertTrue(fed.ok, fed)
                        self.assertTrue(llm.started.wait(timeout=2.0))
                        self.assertTrue((await overlap.coordinator.start_finalize_pcm()).ok)
                        return await overlap.coordinator.settle_finalize()

                    final = asyncio.run(finalize_overlap())
                    self.assertTrue(final.ok, final)
                    self.assertEqual(final.input_state, "finalized")
                    self.assertEqual(final.disposition, "")
                    self.assertEqual(len(engine.calls), 1)
                    duck = channel.take_control_outbound()
                    self.assertEqual(duck.action, "duck")
                    self.assertTrue(channel.mark_control_sent(duck.control_id).ok)
                    self.assertTrue(
                        channel.acknowledge(
                    ...
                    async def wake_on_semantic_change() -> None:
                        waiting = asyncio.create_task(overlap.delivery_channel.wait_activity())
                        await asyncio.sleep(0)
                        self.assertFalse(waiting.done())
                        llm.release.set()
                        await asyncio.wait_for(waiting, timeout=2.0)

                    asyncio.run(wake_on_semantic_change())
                    self.assertTrue(service.wait_idle(timeout=5.0))
                    host = overlap.coordinator.bridge.host
                    turn = host.snapshot.input_turns[request.voice_turn_id]
                    self.assertEqual(turn.disposition.value, "interaction" if decision == "backchannel" else "message")
                    self.assertEqual(len(engine.calls), 1 if decision == "backchannel" else 2)
                    control = channel.take_control_outbound()
                    self.assertIsNotNone(control)
                    self.assertEqual(control.action, "stop" if stop_now else "resume")
                    self.assertFalse(
                        channel.response_outcome().response_terminal, "Keep the old channel until playback ACK"
                    )
                    ...
                    acknowledged = channel.acknowledge(
                        "client.playback.control_ack",
                        {
                            ...
                            "status": "applied",
                            "played_ms": 450,
                            **({"applied_volume": 1.0} if not stop_now else {}),
                        },
                    )
                    self.assertTrue(acknowledged.ok, acknowledged)
                    self.assertEqual(acknowledged.response_terminal, stop_now)
                    if stop_now:
                        self.assertIsNotNone(overlap.delivery_channel.take_outbound())
                    elif decision != "backchannel":
                        self.assertIsNone(
                            overlap.delivery_channel.take_outbound(),
                            "New reply waits for the old unit's real completion",
                        )
                        completed = channel.acknowledge(
                        ...
                        self.assertTrue(completed.response_terminal)
                        self.assertIsNotNone(overlap.delivery_channel.take_outbound())
                    else:
                        self.assertEqual(host.snapshot.speech_units[delivery.speech_unit_id].state.value, "playing")
                        self.assertTrue(overlap.delivery_channel.interaction_committed())
                        messages = []

                        async def send_json(payload: Any) -> None:
                            messages.append(payload)

                        async def deliver_interaction() -> None:
                            transport = VoiceRealtimeCallWebSocketSession(
                                websocket=SimpleNamespace(send_json=send_json),
                                call_factory=None,
                                runtime_metrics=None,
                                log_event=lambda *_args, **_kwargs: None,
                            )
                            transport.delivery_channels[request.voice_turn_id] = overlap.delivery_channel
                            transport.current_request = request
                            await transport._handle_delivery_activity(request.voice_turn_id)
                            self.assertEqual(messages, [], "ASR final must precede interaction settlement")
                            transport.current_request = None
                            await transport._handle_delivery_activity(request.voice_turn_id)
                            await transport._handle_delivery_activity(request.voice_turn_id)
                            self.assertFalse(transport.delivery_channels)

                        asyncio.run(deliver_interaction())
                        self.assertEqual([message["type"] for message in messages], ["server.turn.interaction"])
                        self.assertFalse(overlap.delivery_channel.response_outcome().response_terminal)
                        self.assertNotIn(request.voice_turn_id, [key[1] for key in service._delivery_channels])
    ...
    def test_new_ordinary_input_cancels_old_response_only_after_commit(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manager = self._manager(root)
            tts = _SlowTTSClient()
            service = self._service(root=root, manager=manager, adapter=_Adapter(), tts_client=tts)
            try:
                source = service.create_coordinator(_open_request(output_mode=VOICE_PLAYBACK_OUTPUT_MODE))
                asyncio.run(_commit_realtime_turn(source.coordinator))
                self.assertTrue(tts.started.wait(timeout=2.0))
                host = source.coordinator.bridge.host
                old_id = next(iter(host.snapshot.responses))
                next_turn = service.create_coordinator(
                    _open_request(voice_turn_id="ordinary-next", output_mode=VOICE_PLAYBACK_OUTPUT_MODE)
                )

                async def commit_next() -> None:
                    self.assertTrue((await next_turn.coordinator.open()).ok)
                    self.assertTrue(
                        (await next_turn.coordinator.feed_pcm_frame(b"\x01\x00" * 160, sequence=0, audio_clock_ms=0)).ok
                    )
                    self.assertNotEqual(host.snapshot.responses[old_id].state.value, "cancelled")
                    self.assertTrue((await next_turn.coordinator.start_finalize_pcm()).ok)
                    result = await next_turn.coordinator.settle_finalize()
                    self.assertTrue(result.ok, result)
                    self.assertEqual(result.disposition, "message")

                asyncio.run(commit_next())
                self.assertEqual(host.snapshot.responses[old_id].state.value, "cancelled")
                tts.release.set()
                self.assertTrue(service.wait_idle(timeout=5.0))
                self.assertIsNone(source.delivery_channel.take_outbound())
                self.assertIsNotNone(next_turn.delivery_channel.take_outbound())
            finally:
                tts.release.set()
                service.close()
                manager.close()

    def test_closed_model_sentence_starts_tts_before_metadata_arrives(self) -> None:
        class MetadataDelayedEngine(_ThinkingEngine):
            release_metadata = threading.Event()
            awaiting_metadata = threading.Event()

            def process_voice_turn_stream(self, **kwargs: Any):
                self.calls.append(dict(kwargs))
                tap = _TopLevelJSONStreamTap()
                for chunk in ('{"speech":"\\uD83D', "\\uDE0A第一句？！", '"'):
                    yield from tap.feed(chunk)
                self.awaiting_metadata.set()
                if not self.release_metadata.wait(timeout=5.0):
                    raise RuntimeError("test metadata release timeout")
                yield from tap.feed(',"memory_metadata":{}}')
                yield from tap.finish()
                yield {"type": "final", "payload": {"speech": tap.latest_speech, "memory_metadata": {}}}

        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manager = self._manager(root)
            engine = MetadataDelayedEngine(manager)
            ...
            try:
                ...
                asyncio.run(_commit_realtime_turn(resolved.coordinator))
                self.assertTrue(engine.awaiting_metadata.wait(timeout=2.0))
                self.assertTrue(tts_client.started.wait(timeout=2.0), "TTS must start while metadata is still withheld")
                self.assertFalse(engine.release_metadata.is_set())
                self.assertEqual(tts_client.calls, ["😊第一句？！"])
                ...
                while delivery is None and time.monotonic() < deadline:
                    ...
                    if delivery is None:
                        time.sleep(0.01)
                self.assertIsNotNone(delivery, "First audio must reach playback before final metadata")
                self.assertEqual(delivery.text, "😊第一句？！")
                self.assertFalse(engine.release_metadata.is_set())
                engine.release_metadata.set()
                self.assertTrue(service.wait_idle(timeout=5.0))
                self.assertEqual(
                    tts_client.calls, ["😊第一句？！"], "Final must not synthesize the first sentence twice"
                )
    ...
    def test_restart_during_background_tts_does_not_repeat_the_provider_call(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manager = self._manager(root)
            ...
            try:
                ...
                self.assertEqual(resolved.status, "ready", resolved)
                asyncio.run(_commit_realtime_turn(resolved.coordinator))
                self.assertTrue(tts_client.started.wait(timeout=2.0))
                self.assertTrue(engine.stream_consumed.wait(timeout=2.0))
                recovered_root = root / "restarted"
                from tests.voice_state_snapshot import copy_voice_state
                copy_voice_state(root / "voice-state", recovered_root / "voice-state")
                recovered_tts = _TTSClient()
                restarted = self._service(
                    root=recovered_root,
                    manager=manager,
                    adapter=_Adapter(),
                    tts_client=recovered_tts,
                )
                recovered = restarted.create_coordinator(
                    _open_request(voice_turn_id="after-restart", output_mode=VOICE_PLAYBACK_OUTPUT_MODE)
                )
                self.assertEqual(recovered.status, "unavailable", recovered)
                self.assertEqual(recovered.reason, "voice_tts_outcome_unconfirmed")
                self.assertEqual(recovered_tts.calls, [])
            finally:
                tts_client.release.set()
                service.close()
    ...
    def test_background_tts_failure_settles_without_fake_audio(self) -> None:
        class FailingTTS(_TTSClient):
            async def synthesize(self, text: str) -> bytes:
                self.calls.append(text)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            service = self._service(root=root, manager=manager, adapter=_Adapter(), tts_client=tts_client)
            try:
                resolved = service.create_coordinator(_open_request(output_mode=VOICE_PLAYBACK_OUTPUT_MODE))
                self.assertEqual(resolved.status, "ready", resolved)
                asyncio.run(_commit_realtime_turn(resolved.coordinator))
                ...
                self.assertEqual(len(tts_client.calls), 1)
                self.assertEqual(next(iter(host.snapshot.speech_units.values())).state.value, "failed")
                self.assertFalse(host.snapshot.pending_commands)
                self.assertIsNone(resolved.delivery_channel.take_outbound())
                events = host._host.journal.load_events()
                failures = [event for event in events.events if event.event_kind == "voice.tts.failed"]
                self.assertEqual(len(failures), 1)
                self.assertEqual(failures[0].payload["reason_code"], "voice_tts_synthesis_failed")
                self.assertNotIn("private provider", str(events))
            finally:
                service.close()
                manager.close()

    def test_slow_tts_does_not_block_text_or_cancel_and_discards_late_audio(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manager = self._manager(root)
            engine = _AheadOfTTSThinkingEngine(manager)
            ...
            try:
                ...
                asyncio.run(_commit_realtime_turn(resolved.coordinator))
                self.assertTrue(tts_client.started.wait(timeout=2.0))
                self.assertTrue(engine.stream_consumed.wait(timeout=2.0), "TTS must not stall model delta consumption")
                host = resolved.coordinator.bridge.host
                self.assertEqual(len(host.snapshot.speech_units), len(engine.segments))
                executor = next(iter(service._tts_executors.values()))
                self.assertLessEqual(len(executor._jobs), 2)
                for _ in range(3):
                    host.drive_once()
                receipts = host._host.journal.load_unfinished_commands()
                self.assertTrue(receipts.ok, receipts)
                self.assertTrue(any(record.phase == "executing" for record in receipts.records))
                self.assertEqual(tts_client.calls, [engine.segments[0]])

                def cancel() -> None:
                    try:
                        cancel_results.append(resolved.coordinator.cancel_response(reason="new_user_turn"))
                    finally:
                ...
                cancel_thread.start()
                self.assertTrue(
                    cancellation_finished.wait(timeout=1.0),
                    "Cancellation must complete while the TTS provider is still blocked",
                )
                self.assertEqual(cancel_results[0].status, "accepted", cancel_results)
                self.assertFalse(tts_client.release.is_set())
                tts_client.release.set()
                self.assertTrue(service.wait_idle(timeout=5.0))
                self.assertEqual(tts_client.calls, [engine.segments[0]], "Cancelled queued units must not synthesize")
                self.assertIsNone(resolved.delivery_channel.take_outbound())
                self.assertEqual(next(iter(host.snapshot.responses.values())).state.value, "cancelled")
                events = host._host.journal.load_events()
                self.assertFalse(any(event.event_kind == "voice.tts.ready" for event in events.events))
                self.assertFalse(host.snapshot.pending_commands)
                self.assertFalse(executor._jobs)
            finally:
                tts_client.release.set()
    ...
    def test_bounded_background_tts_drains_all_segments_once_in_order(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manager = self._manager(root)
            engine = _AheadOfTTSThinkingEngine(manager)
            ...
            try:
                ...
                self.assertTrue(tts_client.started.wait(timeout=2.0))
                self.assertTrue(engine.stream_consumed.wait(timeout=2.0))
                self.assertFalse(service.wait_idle(timeout=0.1), "Idle must include outstanding TTS work")
                tts_client.release.set()
                self.assertTrue(service.wait_idle(timeout=5.0))
                self.assertEqual(tts_client.calls, list(engine.segments))
                host = resolved.coordinator.bridge.host
                units = list(host.snapshot.speech_units.values())
                self.assertEqual([unit.state.value for unit in units], ["ready"] * len(engine.segments))
                delivery = resolved.delivery_channel.take_outbound()
                self.assertIsNotNone(delivery)
                self.assertEqual(delivery.text, engine.segments[0])
                self.assertIsNone(resolved.delivery_channel.take_outbound(), "Only the next eligible unit may play")
                executor = next(iter(service._tts_executors.values()))
                self.assertFalse(executor._jobs)
            finally:
                tts_client.release.set()
    ...
    def test_call_scoped_asr_rejects_legacy_session_without_capability_attribute(self) -> None:
        async def exercise() -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    opened = await service.open_call(_open_request(voice_turn_id="voice-turn-legacy-call"))
    ...
    def test_speculative_candidate_waits_for_final_transcript_before_delivery(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIsNone(candidate.delivery_channel.take_presentation())
                ...
                self.assertEqual(
                    candidate.delivery_channel.take_presentation(),
                    {
                        "response_id": adopted.response_id,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                projected_voice_user = next(
                    ...
                    if payload.get("role") == "user" and "medium: voice" in str(payload.get("content") or "")
                )
