...
The local `charpack-core` source now loads `persona.md` in full, without the
legacy 2600-character truncation. The model's overall context window still
applies. Akane's local environment uses an editable installation of the sibling
`charpack-core` repository. Until that change is released in a versioned package,
recreating this environment requires installing that source after the pinned
dependencies: `uv pip install --python .venv/Scripts/python.exe --no-deps -e ../charpack-core`.
Installing the original published 0.1.0 package restores the old truncation;
`test_installed_charpack_preserves_long_persona` detects that regression.
