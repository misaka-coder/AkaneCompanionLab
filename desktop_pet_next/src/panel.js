...
function startProgressInterpolation() {
  ...
  function tick() {
    ...
    const elapsed = state.musicPositionAt > 0
      ...
      : 0;
    const pos = Math.min(state.musicPosition + Math.min(5, elapsed) * state.musicPlaybackRate, state.musicDuration);
...
const els = {
  ...
  mediaStateLabel: document.getElementById("media-state-label"),
...
function createOperationId(prefix = "panel") {
...
function renderMusic() {
  ...
  els.mediaStateLabel.textContent = state.musicPlaying ? "正在播放" : state.musicPlaybackStatus === "paused" ? "已暂停" : "媒体已停止";
...
async function setupEventBridge() {
  ...
  await listen("panel:state-update", (event) => {
    ...
    if (Number.isFinite(s.musicPlaybackRate)) state.musicPlaybackRate = s.musicPlaybackRate;
    if (s.musicPlaybackStatus) state.musicPlaybackStatus = s.musicPlaybackStatus;
