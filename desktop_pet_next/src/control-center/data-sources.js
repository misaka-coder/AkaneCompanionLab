...
const chatBackendActionIds = new Set([
...
const settingsCommandByActionId = Object.freeze({
  ...
  ...Object.fromEntries(Object.entries(SCREEN_OBSERVATION_COMMANDS).map(([id, spec]) => [id, spec.command])),
...
export function createBackendControlCenterSource(options = {}) {
  ...
  const source = {
    ...
    sessionId,
    characterPackId,
    async readToolExposure(overrides = {}) {
      if (!await ensureVerifiedBackend()) return { ok: false, reason: "instance_health_unavailable" };
      const params = { user_id: overrides.sessionId || sessionId, real_user_id: profileUserId,
        character_pack_id: overrides.characterPackId ?? characterPackId };
      const result = await fetchJson(fetchImpl, buildBackendUrl(botBaseUrl, "/capabilities/tool-exposure", params));
      return result.ok ? result.data : { ok: false, reason: result.error || result.status };
    },
    async saveToolExposure(payload, overrides = {}) {
      if (!await ensureVerifiedBackend()) return { ok: false, reason: "instance_health_unavailable" };
      const params = { user_id: overrides.sessionId || sessionId, real_user_id: profileUserId,
        character_pack_id: overrides.characterPackId ?? characterPackId };
      const response = await fetchImpl(buildBackendUrl(botBaseUrl, "/capabilities/tool-exposure", params), {
    ...
    async readSnapshot() {
      ...
      return {
        ...
        controlCenterRuntime: {
          ...
          resourceManifest,
          metrics,
          executionPolicy: { ok: false, status: "unavailable", data: null },
          resourcePolicy: { ok: false, status: "unavailable", data: null },
          capabilitiesCatalog,
    ...
    async readPluginCatalog() {
      ...
      const enrichedPayload = await readPluginConnectionConfigs(fetchImpl, botBaseUrl, payload, profileUserId);
      return { ok: true, status: "available", data: enrichedPayload };
    },
...
function backendActionRunner(actionId) {
  if (chatBackendActionIds.has(actionId)) return runChatBackendAction;
...
async function runChatBackendAction(fetchImpl, baseUrl, actionId, payload = {}, params = {}) {
  const jobId = String(payload.jobId || payload.job_id || "").trim();
  const controlAction = String(payload.action || "").trim().toLowerCase();
  if (!jobId || !["pause", "resume", "stop"].includes(controlAction)) {
    return {
      ok: false,
  ...
  try {
    const response = await fetchImpl(
      buildBackendUrl(baseUrl, `/admin/plugins/jobs/${encodeURIComponent(jobId)}/control`, params),
      {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
    ...
    return {
      ...
      jobId,
      controlAction,
      status: String(result?.status || (ok ? "completed" : `http-${response.status}`)),
      reason: ok ? String(result?.reason || "") : jobControlFailureDetail(result, response.status),
      jobStatus: String(result?.job_status || ""),
      controlState: String(result?.control_state || ""),
      refresh: true
    };
  } catch (error) {
    return {
      ...
      refresh: true,
      error: formatDataSourceError(error),
      reason: "请求没有完成，请检查服务连接后重试"
    };
  }
}

function jobControlFailureDetail(result, httpStatus) {
  const status = String(result?.status || "").trim();
  const reason = String(result?.reason || "").trim();
  if (status === "pause_unavailable") return "当前执行器没有合作式暂停点，请先停止任务";
  if (status === "resume_unavailable") return "当前任务没有处于可恢复的暂停状态";
  if (status === "unknown" || status === "not_found" || httpStatus === 404) return "任务已经不存在或不属于当前会话";
  if (status === "invalid" || httpStatus === 400) return "任务控制请求无效";
...
async function runPluginBackendAction(fetchImpl, baseUrl, actionId, payload = {}, params = {}) {
  if (actionId === CONTROL_CENTER_ACTIONS.abilitiesPluginConnectionSave) {
    return runPluginConnectionBackendAction(fetchImpl, baseUrl, actionId, payload, params);
  }
...
async function runPluginConnectionBackendAction(fetchImpl, baseUrl, actionId, payload = {}, params = {}) {
  const pluginId = String(payload.pluginId || payload.plugin_id || "").trim();
  const connectionName = String(payload.connectionName || payload.connection_name || "").trim();
  ...
  const clearFields = Array.isArray(payload.clearFields)
    ? payload.clearFields.map((item) => String(item || "").trim()).filter(Boolean)
    : Array.isArray(payload.clear_fields)
      ? payload.clear_fields.map((item) => String(item || "").trim()).filter(Boolean)
      : [];
  if (!pluginId || !connectionName || !Number.isInteger(expectedRevision) || expectedRevision < 0) {
    return { ok: false, status: "invalid-payload", actionId, refresh: false, error: "pluginId, connectionName and expectedRevision are required" };
  }
  try {
    const response = await fetchImpl(
      buildBackendUrl(baseUrl, `/admin/plugins/${encodeURIComponent(pluginId)}/connections/${encodeURIComponent(connectionName)}`, {
        ...params,
        profile_user_id: params.profile_user_id || "master"
      }),
      {
        method: "PUT",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
    ...
    return {
      ...result,
      ok,
      status: String(result?.status || (ok ? "configured" : `http-${response.status}`)),
      reason: ok ? "" : pluginConnectionFailureDetail(result, response.status),
      actionId,
      pluginId,
      connectionName,
      refresh: true
...
function pluginConnectionFailureDetail(result, httpStatus) {
  const status = String(result?.status || "").trim();
  const reason = String(result?.reason || "").trim();
  if (status === "conflict" || reason === "connection_revision_conflict") return "配置已经被其他窗口更新，请刷新后再保存";
  if (reason === "connection_values_invalid") return "配置值不符合插件声明，请检查字段";
  if (reason === "connection_clear_update_conflict") return "同一个字段不能同时填写和清除";
  if (reason === "admin_auth_required" || httpStatus === 401 || httpStatus === 403) return "此操作只能从受信控制中心执行";
  return reason || `http-${httpStatus}`;
}
...
async function readPluginConnectionConfigs(fetchImpl, baseUrl, payload, profileUserId) {
  const plugins = Array.isArray(payload?.plugins) ? payload.plugins : [];
  const enriched = await Promise.all(plugins.map(async (plugin) => {
    const pluginId = String(plugin?.plugin_id || "").trim();
    const connections = Array.isArray(plugin?.contributions?.connections) ? plugin.contributions.connections : [];
    if (!pluginId || !connections.length) return plugin;
    const configs = await Promise.all(connections.map(async (connection) => {
      const name = String(connection?.name || "").trim();
      if (!name) return null;
      const result = await fetchJson(fetchImpl, buildBackendUrl(baseUrl, `/admin/plugins/${encodeURIComponent(pluginId)}/connections/${encodeURIComponent(name)}`, { profile_user_id: profileUserId }));
      if (result.ok && result.data && typeof result.data === "object") {
        return { ...result.data, connection: result.data.connection || connection };
      }
      return { ok: false, status: result.status || "unavailable", reason: result.error || "connection_config_unavailable", connection, revision: 0, enabled: false, values: {}, configured_private_fields: [], private_fields: connection.private_fields || [] };
    }));
    return { ...plugin, connection_configs: configs.filter(Boolean) };
  }));
  return { ...payload, plugins: enriched };
}
...
function voiceProviderReasonLabel(reason) {
  const labels = {
    requested_voice_profile_missing: "角色声线档案未配置，保留文字回复",
    ...
    requested_provider_invalid: "角色请求的语音服务标识无效",
    requested_provider_not_ready: "指定的语音服务尚未就绪，保留文字回复",
    text_only_requested: "角色使用仅文字回复",
...
function buildAbilitiesRuntimePatch({ diagnostics, workspace, capabilitiesCatalog, voiceProfilesCatalog, approvalRequestsCatalog, executionPolicy = null, resourcePolicy = null, connected }) {
  const diagnosticsData = asObject(diagnostics);
...
async function tryReadUnifiedSnapshot(fetchImpl, baseUrl, scope = {}) {
  ...
  try {
    ...
    const executionPolicy = unpackUnifiedSnapshotField(runtime.executionPolicy);
    const resourcePolicy = unpackUnifiedSnapshotField(runtime.resourcePolicy);
    ...
    return {
      ...
      abilitiesRuntime: buildAbilitiesRuntimePatch({
        ...
        voiceProfilesCatalog: voiceProfilesCatalog.data,
        approvalRequestsCatalog: approvalRequestsCatalog.data,
        executionPolicy: executionPolicy.data,
        resourcePolicy: resourcePolicy.data,
        connected: health.ok || diagnostics.ok || capabilitiesCatalog.ok || voiceProfilesCatalog.ok || approvalRequestsCatalog.ok
      }),
      advancedRuntime: buildAdvancedRuntimePatch({
        health: health.data,
