...
const interruptionSession = new RealtimeVoiceSession({
  ...
  callbacks: {
    ...
    onPresentation: (payload) => interruptionEvents.push(payload.presentation.emotion)
  },
  scope: {
    Blob: FakeBlob,
    URL: {
...
interruptionSession.handleServerEvent({ type: "server.response.presentation", presentation: { emotion: "happy" } });
assert.deepEqual(interruptionEvents, ["accepted", "skipped", "happy"]);
assert.equal(interruptionSession.responseTerminal, false);
interruptionSession.handleServerEvent({ type: "server.response.completed" });
interruptionSession.handleServerEvent({ type: "server.response.presentation", presentation: { emotion: "late" } });
assert.deepEqual(interruptionEvents, ["accepted", "skipped", "happy"]);
...
const firstCallTurn = callScopedSession.createTurn({
  ...
  callbacks: {
    ...
    onInteraction: () => firstCallTurnEvents.push("interaction"),
    onPresentation: (payload) => callPresentations.push(payload.presentation.emotion)
  }
});
...
callScopedSession.handleServerEvent({ type: "server.response.presentation", voice_turn_id: "unknown", presentation: { emotion: "wrong" } });
callScopedSession.handleServerEvent({ type: "server.response.presentation", voice_turn_id: "call-turn-1", presentation: { emotion: "happy" } });
assert.deepEqual(callPresentations, ["happy"]);
assert.equal(firstCallTurn.responseTerminal, false);
callSocket.emit("message", {
  data: JSON.stringify({
    type: "server.turn.interaction", protocol_version: 2, voice_turn_id: "call-turn-1",
    state: "committed", disposition: "interaction"
  })
});
assert.deepEqual(firstCallTurnEvents, ["ready", "final", "interaction"]);
assert.equal(firstCallTurn.responseTerminal, true);
callScopedSession.handleServerEvent({ type: "server.response.presentation", voice_turn_id: "call-turn-1", presentation: { emotion: "late" } });
assert.deepEqual(callPresentations, ["happy"]);
